...
| Standalone `registerObjectArt` engine wiring | sample-platformer Task 5 review (2026-07-13) | Mod runtime/standalone modules | `ModContext.registerObjectArt` sheets are prepared into `ModRegistrationPlan.preparedObjectArt()` but silently dropped for standalone owners — `OwnerAwareStandaloneModule.wrap` is a passthrough with no `ModArtOverlayProvider` decoration, so the published API is a no-op trap forcing every standalone author to hand-roll a provider (the `sample-platformer` module now does exactly that). Wiring the decoration into the standalone wrap, mirroring `ModBackedGamePatch.getObjectArtProvider()`, makes the published surface behave uniformly. | **Scheduled** — engine-side fix; the sample's hand-rolled provider becomes the regression fixture. |
