...
LZ `Map_Harp` table, the SLZ/SBZ `Map_Bomb` table, the SBZ `Map_Flame` / `Map_Saw` / `Map_Elec` / `Map_ADoor` / `Map_Gird` /
...
have been ratcheted down. The current object-provider budget is 875 handwritten
