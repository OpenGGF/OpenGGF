...
the MZ/SLZ `Map_Fire` table, the GHZ/SLZ `Map_Smash` table, the LZ `Map_Harp`
table, the SBZ `Map_Flame` / `Map_ADoor` / `Map_Gird` / `Map_Trap` /
`Map_Spin` tables, the shared button `Map_But` table, the SBZ2 `Map_FFloor`
table, the shared boss `Map_Eggman` /
...
have been ratcheted down. The current object-provider budget is 1138 handwritten
