...
To avoid showing void to the right of the level edge at widescreen, the camera's horizontal clamp (`Camera.horizontalMaxClamp()`) caps the camera's *right* edge at the level edge: the effective top-left clamp is `max(minX, maxX − (viewportWidth − 320))`, so the camera stops with the level edge on the screen's right edge and the player reaches near the visible right edge at the wall. At native width this equals `maxX` (byte-identical). A fully locked camera (`minX == maxX`, e.g. a boss arena) is left pinned (the clamp never drops below `minX`), and a wrapped/unbounded range (`maxX < minX`, e.g. SCZ) is left as-is. `getMaxX()` still returns the stored native `maxX`, so the player boundary above is unaffected.
