...
shared boss `Map_Eggman` / `Map_BossItems` tables, the SBZ2/FZ `Map_SEgg` table,
plus the Final Zone `Map_EggCyl` / `Map_PLaunch` / `Map_Plasma` /
`Map_FZLegs` / `Map_FZDamaged` boss mapping slice, now load from the
user-supplied ROM and their guard budgets have been ratcheted down. The current
object-provider budget is 1387 handwritten mapping pieces; Sonic 1 boss mappings
are ROM-backed and their separate handwritten budget is zero.
