...
the MZ/SLZ `Map_Fire` table, the MZ `Map_Bas` / `Map_Glass` / `Map_Geyser` tables, the GHZ `Map_Hel` / `Map_Swing_GHZ` tables, the SLZ `Map_Swing_SLZ` table, the SYZ
...
table, the SBZ Running Disc `Map_Disc` table, the LZ Breakable Pole
`Map_Pole` table, the MZ Brick `Map_Brick` table,
the SYZ Spinning Light `Map_Light` table, the MZ Smashable Green Block
`Map_Smab` table, the MZ/SLZ/SBZ Collapsing Floor `Map_CFlo` table, the SBZ vanishing platform `Map_VanP` table, the SBZ2
...
have been ratcheted down. The current object-provider budget is 574 handwritten
