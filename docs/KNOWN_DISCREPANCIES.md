...
LZ `Map_Jaws` / `Map_Burro` / `Map_Flap` / `Map_WFall` tables, the GHZ/SLZ
`Map_Smash` table, the LZ `Map_Harp` table, the shared button `Map_But` table,
the SBZ2 `Map_FFloor` table, the shared boss `Map_Eggman` /
...
have been ratcheted down. The current object-provider budget is 1287 handwritten
