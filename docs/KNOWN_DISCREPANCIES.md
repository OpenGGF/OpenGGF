...
`Map_Bump` / `Map_Spring` / `Map_Roll` / `Map_Yad` tables, the GHZ `Map_Crab` / `Map_Moto` / `Map_Newt` tables, the GHZ/SLZ `Map_Smash` table, the
...
have been ratcheted down. The current object-provider budget is 846 handwritten
