...
the MZ/SLZ `Map_Fire` table, the MZ `Map_Glass` table, the GHZ/SLZ
`Map_Smash` table, the LZ `Map_Harp` table, the SBZ `Map_Flame` / `Map_Saw` /
`Map_Elec` / `Map_ADoor` / `Map_Gird` /
...
have been ratcheted down. The current object-provider budget is 1068 handwritten
