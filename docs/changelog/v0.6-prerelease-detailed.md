...
- **docs(audio): resolve the loader-scoped portion of the S3K SMPS meta-command finding with ROM reachability
  evidence.** Sequencing every S&K-loader-supported S&K music entry (`0x01-0x33`), S3 music
  entry (`0x01-0x32`), and SFX entry (`0x33-0xDB`) reaches no `FF 01`
  (`SND_CMD`), `FF 02` (`MUS_PAUSE`), or `FF 03` (`COPY_MEM`) command. The
  `Sonic3kCoordFlagHandler` cases now state that they consume operands only to
  preserve alignment for custom/imported streams; they do not pretend to
  implement native sound dispatch, all-track halt/resume, or shared-Z80-memory
  copying. S3-native SFX/alias coverage remains open. The ROM-backed guard and native Z80 contracts are recorded in
