...
| Scrap Brain 1-3 | Low-medium risk | High risk | Low risk | Teleporter/junction ownership, running-disc attachment, spinning-conveyor rider release, pit-transition authority, and direct SBZ3-to-FZ team locking now cover arbitrary sidekicks without changing main progression. No spin-dash route gate was found. Late camera-X triggers around pits and arena boundaries still need full-route alternate-width validation. |
| Final Zone | Low-medium risk | Medium risk | Low risk | Solids and boss contacts already accept all players; escape containment now locks/caps main plus every sidekick, and boss escape visibility uses the active 320/352/400/528/800 viewport before ending cleanup. Boss AI/progression deliberately remains main-oriented. Roll-state fallback permits donated rolling attacks; no spin-dash dependency was found. Full alternate-width boss playthroughs remain outstanding. |
| Ending | Low risk | Medium risk | Low risk | Main-player thresholds still own progression, while bootstrap, forced running, stop, and hide handoff apply to every registered sidekick. WAIT presentation resolves through each donated animation profile. Alternate-width visual captures are still needed to rule out exposed off-stage art. |
