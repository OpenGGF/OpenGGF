...
- AutomaticTunnel retains native P1/P2 state-machine order, then processes identity-keyed extension routes. Promotion, demotion, omission, death, unload, persistence, and rewind replacement keep state with the actor and release only tunnel-owned control.
- CNZ Cylinder retains native standing bits, P1/P2 diagnostics, and update order. Additional riders use identity-keyed extension slots plus an aggregate extension-standing bit solely for shared cylinder motion; cleanup is ownership-safe and rewindable.
...
- The HCZ miniboss vortex processes the full ordered roster and records only control it acquired. Omission, death, unload, and rewind replacement release the actual owned actor without clearing unrelated object control.
