...
| `$AA` Act1 miniboss | concrete | mandatory boss; one S3KL placement at `$2F00,$05E0`; object-owned camera/plunger arena; scripted six-cycle self-damage | `Map_FBZMiniboss` (18 frames; pieces `4,1,1,2,2,2,2,4,6,6,6,6,6,6,6,6,6,2`), direct `ArtKosM_FBZMiniboss`, `Pal_FBZMiniboss`, shared raw boss-explosion PLC; S3 PLC `$5E` is unused | closest native-pair aimer/terminal touch; P1-only lunge target; only plunger status bit 3 (P1 standing) starts the fight, while P2/extras may ride and collide without setting root bit 0; SKL `$AA` remains Hyudoro | placement + seven-child FNFO table + two five-link cyclic FNFO chains + transient palette FNFO + defeat helper/5 animals/5 fragments FNFO + fade FFO; actual after-current EndSign child | `TestFbzAct1Miniboss`, `TestFbzMinibossChildren`, `TestFbzMinibossDefeatChildren`, `TestFbzMinibossRewind`, `TestFbzAct1RouteHeadless`, `TestFbzMinibossArtShape` | complete; completeness decreases exactly one; `$78` music wait fires on wait update 121 and cover `$20/$20/$40` phases consume 33/33/65 wait updates; boss does not set `Events_fg_5`, later level-results flow does; real route covers sign/results handoff, restart, multi-sidekicks, widescreen, and donation-neutral standing activation |
| `$AB` Act2 subboss | concrete | mandatory traversal subboss at `$2B40,$05F0`; P1-only `$18` trigger; seven laser cycles with six left-anchor advances | `Map_FBZ2Subboss`, `Pal_FBZ2Subboss`, character-selected Robotnik/EggRobo stand/run art, generic `Map_SpriteMask` frame 4, cloud→pillar KosM defeat queue, then raw monitor restoration | native P1 aim/trigger and P1/P2 laser-result branch; ordinary touch and side-solid participation safely extend to engine players without granting extras state authority; SKL `$AB` remains SOZ capsule/Hyudoro | independent fade FFO; machine/character, mask, corner and side-solid FNFO prefix tables; transient laser; independent global rumble and real-SST explosion controllers | `TestFbzAct2Subboss`, `TestFbz2SubbossCharacterArt`, `TestFbz2SubbossArtHandoff`, `TestFbz2SubbossRewind`, `TestFbzAct2TraversalPreboss` | complete object graph and art handoff; rewind preserves exact partial prefixes and raw impact callbacks; native-start traversal is proven to frame 11,900 and starpost #5 through the lower magnetic section, while continuous arena entry remains a final BK2 acceptance gate; no event-stage shortcut or spindash-specific donation workaround |
| `$AC` Act2 end boss | concrete in S3KL; SKL mapping preserved | mandatory eight-hit boss spawned once after the retained Plane-B refresh; native Sonic/Tails and Knuckles/EggRobo branches | `Map_FBZEndBoss`, `Map_FBZEndBossFlame`, `Pal_FBZEndBoss`, PLC `$6F`, EggRobo head KosM art, boss-explosion art, and ordered exit door→hall KosM aftermath | closest native P1/P2 targeting with P1 character authority; hazards extend to all engine players without changing native aim, timers, hit cadence, or phase gates | root plus ship, head, weapon, two arms, joints, eight links, nine-flame volley, arm/root debris, infinite subtype-4 explosion controller, escape flame, capsule, P2 lock, and gradual camera workers; every allocation is prefix-retaining/no-heal | `TestFbzEndBoss`, `TestFbzEndBossChildren`, `TestFbzEndBossAudioAndPlc`, `TestFbzEndBossFormalCorrections`, `TestFbzEndBossRewind` | complete Task 16 graph, combat, defeat, rewind, capsule gate, and exit-ready handoff; Task 17 owns exit door/hall presentation and forced transition, and Task 20 owns continuous BizHawk/BK2 route parity |
