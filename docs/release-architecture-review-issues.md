...
| REL-008 | deferred | Trace replay | Legacy S3K AIZ trace bootstrap uses fixture identity and trace-shape phase heuristics. | `src/main/java/com/openggf/trace/TraceReplayBootstrap.java` | Removed the trace-row primary-state substitution path: `capturePrimaryReplayStateForComparison(...)` now reports live engine sprite state only. The legacy fixture-identity predicate remains bounded and diagnostic-only until the AIZ intro trace is regenerated or replaced with ROM-state phase metadata, so this is deferred release debt rather than full trace-policy proof. |
