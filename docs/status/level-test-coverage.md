...
| S3K | `S3K_LAVA_REEF_1` | [Act 1 matrix](../architecture/validation/levels/s3k-lrz-act1.md) - v2 bring-up through most of slice 3c: 609 placements with 98 still placeholders (239 at the slice 0 baseline), 331 live rings, census ratchet in place; scroll, animated tiles, rock sprites, lava blocks, the `$1E` dash elevator, the `$19`/`$1A`/`$1C`/`$1D` doors, buttons and shooting triggers, the `$15` corkscrew and `$16` wall ride, and the `$17` sinking rock, `$18` falling spike, `$1B` fireball launcher, `$1F` lava fall and `$20` swinging spike ball land, while `$21`, `$22`, the rock crusher, badniks, the miniboss and the seamless handover are open; the cold controller-driven route on the fixture's own recorded input matches the native rows exactly to frame 636 and reaches x 4301 of about 11,000, its first divergence being a one-pixel moving-platform jump ordering in the shared solid path rather than missing content. Wide-viewport and donor rows are owed for every slice 3a/3b/3c class |
| S3K | `S3K_LAVA_REEF_2` | [Act 2 matrix](../architecture/validation/levels/s3k-lrz-act2.md) - v2 bring-up through most of slice 3c: 455 placements with 240 still placeholders (281 at the slice 0 baseline), 281 live rings; scroll, animated tiles, rock sprites and the act 2 `$19`/`$1C`/`$16`/`$20` door, horizontal-button, wall-ride and swinging-spike-ball skins land (registered, none yet exercised on an act 2 route), while the rest of the objects, the Death Egg background sprite and both exits are open |
