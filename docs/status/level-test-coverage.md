...
| S3K | `S3K_DEZ_BOSS` | [Final boss matrix](../architecture/validation/levels/s3k-dez-final-boss.md) — v2 bring-up started; `$1700` currently loads the DEZ3 layout with no arena floor and the player falls out and dies. Floor/collapse, scroll, laser-upload, retained-plane, hand/finger, vulnerable-core, mouth/beam, fireball and emerald components now have focused checks and a distinct rewind runtime owner; connected screen initialization, retained redraws, boss and exit remain open |
