...
| S3K | `S3K_SKY_SANCTUARY_1` | [Act 1 matrix](../architecture/validation/levels/s3k-ssz-act1.md) — v2 bring-up in progress; placement census, arrival, act-1 bounds, the opening cutscene, the background (both modes, clouds and solid cloud platforms), the animated tiles, every act-1 traversal family and the EggRobo, the death/checkpoint lifecycle and the Green Hill recreation (including its hit window, three-colour flash, ball hitbox and defeat scatter, all driven through the real touch pass) are covered; the Metropolis and Mecha Sonic fights and the Death Egg launch open. No fight is compared against native rows yet, and clip 17 predates the Green Hill review fixes |
