...
| S3K | `S3K_DEATH_EGG_1` | [Act 1 matrix](../architecture/validation/levels/s3k-dez-act1.md) — v2 bring-up started. Level load, music, intro run, shared objects, the `$5A` gravity tube, the `$5F` turbine corridor, the `$61` gravity puzzle inside it, the `$A4` Spikebonkers, the `$55` energy bridges and the `$A5` Chainspikes (192 of 365 objects concrete). Slice 3 is complete and slice 4 is in progress. The corridor jams at x ~ `$2636` so `$61`'s launch and panels are implemented but unreachable in play ([known bug](s3k-known-bugs.md)); the ten placeholder `$60` `Obj_DEZBumperWall` placements are the named suspect. Presentation, the remaining objects, miniboss, cold route, rewind and native all open |
