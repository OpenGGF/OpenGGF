...
| S3K | `S3K_LAVA_REEF_1` | [Act 1 matrix](../architecture/validation/levels/s3k-lrz-act1.md) - v2 bring-up with slices 3 and 4 complete: 609 placements with **1** still a placeholder (239 at the slice 0 baseline), 331 live rings, census ratchet in place; scroll, animated tiles, rock sprites, lava blocks and every slice 3 class land - `$15`-`$22` plus the `$9C` rock crusher with its timer child, eight hit pieces and the `LRZ1_ScreenEvent` layout edits it requests - and all three badniks, `$99` Fireworm, `$9A` Iwamodoki and `$9B` Toxomister; the miniboss and the seamless handover are open. `TestS3kLrzCompatibilityMatrix` now carries the breadth rows: four rosters (Sonic, Sonic + Tails, Tails, Knuckles), 320 and 400, and the S1 donor, each asserting the live roster and a ready ROM-backed renderer for every art key a slice 3/4 class draws from. The cold controller-driven route on the fixture's own recorded input matches the native rows exactly to **row 856** (was 636) now that the sinking rock keeps its own standing bit until its own checkpoint; the open-loop reach is x 4029, and that reach is not a progress measure -- quote the exact-match row. Owed: cold-route rewind spots for `$1A`, `$1D`, `$9C`, `$9A` and `$99`, `$18`'s landing boundary, and `sub_42EC0` on a route |
| S3K | `S3K_LAVA_REEF_2` | [Act 2 matrix](../architecture/validation/levels/s3k-lrz-act2.md) - v2 bring-up carrying act 1's shared classes: 455 placements with 188 still placeholders (281 at the slice 0 baseline), 281 live rings; scroll, animated tiles, rock sprites and the act 2 `$6E`/`$19`/`$1C`/`$16`/`$17`/`$20`/`$99`/`$9A`/`$9B` skins land, with the act 2 art keys now asserted ready at 320 and 400 by `TestS3kLrzCompatibilityMatrix`, though none is yet exercised on an act 2 route; the rest of the objects, the Death Egg background sprite and both exits are open |
...
| S3K | `S3K_DEATH_EGG_1` | [Act 1 matrix](../architecture/validation/levels/s3k-dez-act1.md) — v2 bring-up started. Level load, music, intro run, shared objects, the `$5A` gravity tube, the `$5F` turbine corridor, the `$61` gravity puzzle inside it, the `$A4` Spikebonkers, the `$55` energy bridges and the `$A5` Chainspikes and `$60` bumper walls (202 of 365 objects concrete). Slice 3 is complete and slice 4 is in progress. The corridor jams at x ~ `$2636` so `$61`'s launch and panels are implemented but unreachable in play ([known bug](s3k-known-bugs.md)); Implementing all ten `$60` bumper walls did not move the corridor frontier; its movement/lifetime behavior still needs investigation. Presentation, the remaining objects, miniboss, cold route, rewind and native all open |
| S3K | `S3K_DEATH_EGG_2` | [Act 2 matrix](../architecture/validation/levels/s3k-dez-act2.md) — v2 bring-up in progress. Reverse gravity is implemented for the player, shields, rings, solid objects, springs and the sidekick (93 of 116 ROM references; the 11 open group A-I rows are in [s3k-known-bugs](s3k-known-bugs.md)), every writer and reader of the flag in the Death Egg object set is concrete (`$5B`, `$58`, `$59`, `$5A`, `$5C`), and slice 4 has added the `$A4` Spikebonkers, `$5D` retracting springs, `$55` energy bridges and `$A5` Chainspikes — 262 of 494 objects. Act 2 has a **1256-frame route frontier** from the first frame of free play, exact in player position, camera and rings and ratcheted in `TestS3kDezColdRoutes` (390 → 472 → 527 → 616 → 1256 as those four landed); the first divergence is now a one-pixel `x` lag on a shared `$08` platform ride, not a Death Egg object. A cold `$B01` route is not comparable against the committed movie until the act 2 entrance sequence is implemented. The seamless act change, the end boss and every breadth row are open |
...
| S3K | `S3K_LRZ_BOSS` | [Boss-act matrix](../architecture/validation/levels/s3k-lrz-boss.md) - 35 placements with zero placeholders, 52 live rings; carry, checkpoint, autoscroll, platform generation and lava presentation implemented. A 900-frame native-input checkpoint approach has no hurt/death and matches P1 motion/camera after its first positioned-load frame. Platform/debris removed-graph replay and partial allocation are checked. The working-tree end-boss candidate reaches capsule/results and `$1601` from a fire-shield checkpoint, with four full-world restore/replay spots; cold flash entry now passes five-width fade/release and restore/forward checks; its extended route exposed bonus-return ordering and a mistaken stack-write/player-position interpretation. Corrected route evidence, capsule slot fidelity and strict parity remain open |
