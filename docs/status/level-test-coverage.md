...
| S3K | `S3K_DOOMSDAY` | [Zone matrix](../architecture/validation/levels/s3k-ddz.md) — in progress; seeded Sonic route matches native; fresh Super routes complete320/800 with fight/wrap/exit replay; death/restart covered; Hyper-star display-list/art-queue phase, retained HUD redraw and recording exit freeze corrected; full cold native320 Sonic+Tails now completes DEZ1→DEZ2→final→DDZ with boot-only emeralds in64648 inputs; remaining native presentation, incoming width/donor breadth and lifecycle isolation open |
