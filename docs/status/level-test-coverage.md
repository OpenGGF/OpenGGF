...
| S3K | `S3K_LAVA_REEF_1` | [Act 1 matrix](../architecture/validation/levels/s3k-lrz-act1.md) - v2 bring-up through slice 3b: 609 placements with 171 still placeholders (239 at the slice 0 baseline), 331 live rings, census ratchet in place; scroll, animated tiles, rock sprites, lava blocks, the `$1E` dash elevator and the `$19`/`$1A`/`$1C`/`$1D` doors, buttons and shooting triggers land, and the `$15` corkscrew, while `$16`, the hazard families, the rock crusher, badniks, the miniboss and the seamless handover are open; a cold controller-driven route from the level start reaches x 1909 of about 11,000 and stalls on phase rather than on missing content |
| S3K | `S3K_LAVA_REEF_2` | [Act 2 matrix](../architecture/validation/levels/s3k-lrz-act2.md) - v2 bring-up through slice 3b: 455 placements with 255 still placeholders (281 at the slice 0 baseline), 281 live rings; scroll, animated tiles, rock sprites and the act 2 `$19`/`$1C` door and horizontal-button skins land (registered, not yet exercised on an act 2 route), while the rest of the objects, the Death Egg background sprite and both exits are open |
