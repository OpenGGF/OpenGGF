...
| S3K | `S3K_LAVA_REEF_1` | [Act 1 matrix](../architecture/validation/levels/s3k-lrz-act1.md) - v2 bring-up with slice 3 complete and slice 4 started: 609 placements with 21 still placeholders (239 at the slice 0 baseline), 331 live rings, census ratchet in place; scroll, animated tiles, rock sprites, lava blocks and every slice 3 class land - `$15`-`$22` plus the `$9C` rock crusher with its timer child, eight hit pieces and the `LRZ1_ScreenEvent` layout edits it requests - and slice 4's `$9A` Iwamodoki and `$9B` Toxomister, while `$99`, the miniboss and the seamless handover are open. The cold controller-driven route on the fixture's own recorded input still matches the native rows exactly to frame 636, but its open-loop reach fell from x 4301 to x 4029 with a death at frame 3649: that input is the fixture's own native input, which native survives, so the reach only measures how long the engine's replay survives its frame-637 phase error and will keep falling as the zone fills in. Quote the exact-match frame, not the reach. Wide-viewport and donor rows are owed for every slice 3 class |
| S3K | `S3K_LAVA_REEF_2` | [Act 2 matrix](../architecture/validation/levels/s3k-lrz-act2.md) - v2 bring-up carrying act 1's shared classes: 455 placements with 197 still placeholders (281 at the slice 0 baseline), 281 live rings; scroll, animated tiles, rock sprites and the act 2 `$6E`/`$19`/`$1C`/`$16`/`$20`/`$9A`/`$9B` skins land (registered, none yet exercised on an act 2 route), while the rest of the objects, the Death Egg background sprite and both exits are open |
