...
- **Suspected cause** — Not the `$5F` object: its blow and both steering directions are confirmed working in the same captures. Two candidates, neither ruled in. The ten act 1 `$60` `Obj_DEZBumperWall` placements are still placeholders, and two of them bracket the jam exactly (`$2600` at y `$07C0` and y `$08C0`); in the ROM they are solid bumpers that fire the player away through the shared `loc_49850`, so their absence changes what the corridor's occupant collides with. Alternatively the engine's level terrain differs here, or `move` + `resolveAirCollision` clamps a blown player where the ROM's `MoveSprite2` + `SonicKnux_DoLevelCollision` would push them out and let them continue.
- **Removal condition** — A native BizHawk capture of a player blown through the act 1 turbine room says whether the ROM passes `$2636` and at what height. If it does, either `$60` lands and the corridor clears, or the terrain/push-out difference is found and fixed; either way the `$61` launch and panel clip is filmed and the act 1 validation matrix's `$61` row gains its visual claim.
