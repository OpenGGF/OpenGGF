...
| RRF-052 | High | fixed | Trace policy | S3K AIZ pre-level replay now uses a generic `pre_level_intro_prefix` fixture capability instead of a legacy game/zone/act/checkpoint identity predicate. | `TraceReplayBootstrap.java`, `TraceMetadata.java`, trace policy tests/docs |
...
| RRF-055 | Medium | fixed | S3K animated tiles | LRZ AniPLC selection now uses `Sonic3kZoneIds.ZONE_LRZ` so Lava Reef Act 1 loads its ROM AniPLC scripts. | `Sonic3kPatternAnimator.java`, `Sonic3kZoneIds.java`, animation tests |
| RRF-056 | Medium | fixed | Object lifecycle | AIZ boss/cutscene dynamic object spawns now use `spawnChild(...)` or `spawnFreeChild(...)`, and the raw-spawn debt discrepancy entry was removed. | `AizBattleshipInstance.java`, `AizEndBossInstance.java`, `AizMinibossInstance.java`, `CutsceneKnucklesRockChild.java`, object guard tests |
...
| RRF-058 | Medium | fixed | Object lifecycle | CNZ ring/bomb prize objects now use `AbstractObjectInstance` destroyed state instead of local shadow fields. | `RingPrizeObjectInstance.java`, `BombPrizeObjectInstance.java`, object lifecycle tests |
