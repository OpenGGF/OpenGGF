...
| RRF-035 | High | open | Trace policy | S3K trace replay reads BK2 input without the same trace-input alignment validation used by non-S3K replay. | `AbstractTraceReplayTest.java`, trace tests |
| RRF-036 | High | open | Trace policy | S3K ring-count comparison can rewrite the expected trace row from next-row engine-matching diagnostics before comparison. | `TraceReplayBootstrap.java`, `TraceFrame.java`, `AbstractTraceReplayTest.java`, trace guard tests |
| RRF-037 | High | open | Rendering parity | S3K animated tile phases can read previous-frame parallax/deform runtime state because animated pattern updates run before parallax state publication. | `LevelRenderer.java`, `Sonic3kPatternAnimator.java`, scroll handlers, render tests |
| RRF-038 | High | open | Coordinate semantics | AIZ hollow-tree terrain reveal compares ROM `Player_1+y_pos` using top-left `getY()` instead of centre `getCentreY()`. | `AizHollowTreeObjectInstance.java`, AIZ object tests |
| RRF-039 | High | open | Runtime assets | S3K runtime object mappings still include hardcoded/transcribed mapping data instead of ROM-parsed mapping frames. | `Sonic3kObjectArt.java`, `Sonic3kObjectArtProvider.java`, asset guard tests |
| RRF-040 | High | open | Release policy | ROM/asset inclusion policy ignores only root `*.gen`/`*.bin` and lacks hook/CI denylist checks for ROM-like files elsewhere. | `.gitignore`, `.githooks/validate-policy.*`, `TestBuildToolingGuard.java` |
