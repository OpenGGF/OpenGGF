...
| RRF-048 | High | open | Release trace gate | Release trace coverage validation enumerates `Test*.java` files that the Maven `trace-replay` profile intentionally excludes, so release CI can fail on missing reports for skipped debug/probe fixtures. | `.github/workflows/release.yml`, `pom.xml`, build tooling tests |
| RRF-049 | High | open | macOS packaging | macOS native packages export editable `config.yaml` beside the `.app`, but native config resolution prefers the executable directory inside `OpenGGF.app/Contents/MacOS`. | `SonicConfigurationService.java`, `assemble-macos-app.sh`, release workflow/tests |
| RRF-050 | High | open | Runtime ownership | Recycled `GameplayModeContext` instances can report runtime-ready after core-manager reattach while stale level managers/shared registries are still non-null from the previous session. | `GameplayModeContext.java`, session lifecycle tests |
| RRF-051 | Medium | open | Runtime ownership | `UiRenderPipeline` retains the previous `HudRenderManager` after gameplay/editor teardown because runtime reference clearing only resets camera/fade. | `GraphicsManager.java`, `UiRenderPipeline.java`, session lifecycle tests |
...
| RRF-053 | High | open | Test/session lifecycle | `GroundSensor` static level-manager override can survive reset paths and direct later tests/sessions at stale level state. | `GroundSensor.java`, `TestEnvironment.java`, lifecycle tests |
