...
| RRF-014 | Medium | deferred | Runtime assets | S1 LZ/SBZ conveyor child spawn tables now load from ROM `ObjPos*Platform_Index` data and the ratchet budgets were tightened; remaining S1 palette cycles, conveyor waypoint groups, bridge bend tables, and handwritten object/boss mapping pieces still require broader ROM-backed migration. | `Sonic1ObjectPlacement.java`, `Sonic1LZConveyorObjectInstance.java`, `Sonic1SpinConveyorObjectInstance.java`, `TestArchitecturalSourceGuard.java`, related S1 assets |
