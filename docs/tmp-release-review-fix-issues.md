...
| RRF-014 | Medium | deferred | Runtime assets | S1 palette cycles and LZ/SBZ conveyor child spawn tables now load from ROM data and the ratchet budgets were tightened; remaining S1 conveyor waypoint groups, bridge bend tables, and handwritten object/boss mapping pieces still require broader ROM-backed migration. | `Sonic1PaletteCycler.java`, `Sonic1ObjectPlacement.java`, `Sonic1LZConveyorObjectInstance.java`, `Sonic1SpinConveyorObjectInstance.java`, `TestArchitecturalSourceGuard.java`, related S1 assets |
