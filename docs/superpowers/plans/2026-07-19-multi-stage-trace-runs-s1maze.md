...
S1 has **no** `Special_bonus_entry_flag` / `Saved_X_pos` analogs: SS entry is discriminated purely by the `$0C -> $10` mode edge — the giant-ring flash object writes `#id_Special` to `v_gamemode` directly while GM_Level is running (`docs/s1disasm/_incObj/4B, 7C Giant Ring and Flash.asm:201`; no results screen or `$8C` intermediate exists between `$0C` and `$10`). The SS results tally runs at SS **exit** (under `$10` — see behavior contract #6), and SS exit returns to the **next act**, not to a saved position.
