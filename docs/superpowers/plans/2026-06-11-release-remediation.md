...
- HCZ complete-run progression: frames 125, 374, 624, 896, 898, 940, 953, 957, 973, 974, 1020, 1581, 1671, 2486, 2501, 2635, 2801, 2894, 2976, and 3066 have been cleared or advanced by the release-remediation sidekick/object slices. The current HCZ first error is frame 3124 `y` (`expected=0x05E7`, `actual=0x05E5`) with `camera_y` also 2 px high in the engine (`expected=0x05A7`, `actual=0x05A5`). Player `x`, speeds, status, rings, and sidekick CPU fields match, so the active owner is the vertical `Obj_HCZWaterWall` rise/eruption handoff and spray/debris cluster.
...
- Remaining SK-1 verification: S3K complete-run trace coverage for fresh sidekick spawn/init-only frame and dormant park semantics. HCZ frame-2894 sidekick follow-history jump-edge publication is covered and advanced; HCZ now needs ROM-state triage of the frame-3124 vertical water-wall rise/eruption frontier, while ICZ/LBZ/MGZ complete-run coverage remains outstanding.
