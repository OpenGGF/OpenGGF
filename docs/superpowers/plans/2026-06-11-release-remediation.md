...
- HCZ complete-run progression: frames 125, 374, 624, 896, 898, 940, 953, 957, 973, 974, 1020, 1581, 1671, 2486, 2501, 2635, 2801, 2894, and 2976 have been cleared or advanced by the release-remediation sidekick/object slices. The current HCZ first error is frame 3066 `y` (`expected=0x07B7`, `actual=0x07BF`) with `camera_y` also 8 px low in the engine (`expected=0x0777`, `actual=0x077F`). Player `x`, speeds, status, rings, and sidekick CPU fields match, so the active owner is the object-controlled HCZ water-wall / air-countdown cluster around `Obj_HCZWaterWall` and the fixed air-countdown child.
...
- Remaining SK-1 verification: S3K complete-run trace coverage for fresh sidekick spawn/init-only frame and dormant park semantics. HCZ frame-2894 sidekick follow-history jump-edge publication is covered and advanced; HCZ now needs ROM-state triage of the frame-3066 object-controlled water-wall / air-countdown frontier, while ICZ/LBZ/MGZ complete-run coverage remains outstanding.
