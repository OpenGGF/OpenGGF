...
- HCZ complete-run progression: frames 125, 374, 624, 896, 898, 940, 953, 957, 973, 974, 1020, 1581, 1671, 2486, 2501, 2635, 2801, 2894, 2976, 3066, 3124, 3318, and 3355 have been cleared or advanced by the release-remediation sidekick/object slices. The current HCZ first error is frame 3850 `tails_y` (`expected=0x0470`, `actual=0x0471`), with `tails_rolling` still true in-engine while the ROM has cleared it. The active owner is now a native-P2 landing/state handoff around nearby path-swap and floating-platform objects, after the HCZ conveyor coarse-back cull was corrected.
...
- Remaining SK-1 verification: S3K complete-run trace coverage for fresh sidekick spawn/init-only frame and dormant park semantics. HCZ frame-2894 sidekick follow-history jump-edge publication, frame-3318 conveyor release center preservation, and frame-3355 conveyor coarse-back culling are covered and advanced; HCZ now needs ROM-state triage of the frame-3850 native-P2 landing/state frontier, while ICZ/LBZ/MGZ complete-run coverage remains outstanding.
