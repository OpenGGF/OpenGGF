...
- HCZ complete-run progression: frames 125, 374, 624, 896, 898, 940, 953, 957, 973, 974, 1020, 1581, 1671, 2486, 2501, 2635, 2801, 2894, 2976, 3066, 3124, and 3318 have been cleared or advanced by the release-remediation sidekick/object slices. The current HCZ first error is frame 3355 `tails_y_speed` (`expected=0x0000`, `actual=0x0318`), with `tails_x_speed` and `tails_y` diverging on the same frame. The active owner is now a sidekick landing / fixed air-countdown object handoff around nearby `0x0002CFA8` air-countdown slots and sidekick follow physics.
...
- Remaining SK-1 verification: S3K complete-run trace coverage for fresh sidekick spawn/init-only frame and dormant park semantics. HCZ frame-2894 sidekick follow-history jump-edge publication and frame-3318 conveyor release center preservation are covered and advanced; HCZ now needs ROM-state triage of the frame-3355 sidekick landing / air-countdown frontier, while ICZ/LBZ/MGZ complete-run coverage remains outstanding.
