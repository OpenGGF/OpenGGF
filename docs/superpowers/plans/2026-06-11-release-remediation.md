...
- HCZ complete-run progression: frames 125, 374, 624, 896, 898, 940, 953, 957, 973, 974, 1020, 1581, 1671, 2486, 2501, 2635, 2801, and 2894 have been cleared or advanced by the release-remediation sidekick/object slices. The current HCZ first error is frame 2976 `y` (`expected=0x0A4E`, `actual=0x0A4B`) with `camera_y` also 3 px high in the engine (`expected=0x0A10`, `actual=0x0A0D`). Player `x`, velocities, rings, sidekick kinematics, and sidekick Ctrl_2 fields match, so the active owner is the main-player vertical/camera path around the post-breakable-bar underwater debris/air-countdown cluster.
...
- Remaining SK-1 verification: S3K complete-run trace coverage for fresh sidekick spawn/init-only frame and dormant park semantics. HCZ frame-2894 sidekick follow-history jump-edge publication is covered and advanced; HCZ now needs ROM-state triage of the frame-2976 main-player vertical/camera frontier, while ICZ/LBZ/MGZ complete-run coverage remains outstanding.
