...
- HCZ complete-run progression: frames 125, 374, 624, 896, 898, 940, 953, 957, 973, 974, 1020, 1581, 1671, 2486, 2501, 2635, 2801, 2894, 2976, 3066, and 3124 have been cleared or advanced by the release-remediation sidekick/object slices. The current HCZ first error is frame 3318 `tails_y` (`expected=0x0349`, `actual=0x0348`). Player position, speeds, camera, rings, sidekick CPU routine, sidekick CPU target/input fields, and `tails_y_speed` match, so the active owner is a sidekick post-water-wall/conveyor interaction parity issue around `HCZConveyorBeltObjectInstance` and the sidekick follow/physics handoff.
...
- Remaining SK-1 verification: S3K complete-run trace coverage for fresh sidekick spawn/init-only frame and dormant park semantics. HCZ frame-2894 sidekick follow-history jump-edge publication is covered and advanced; HCZ now needs ROM-state triage of the frame-3318 post-water-wall sidekick/conveyor frontier, while ICZ/LBZ/MGZ complete-run coverage remains outstanding.
