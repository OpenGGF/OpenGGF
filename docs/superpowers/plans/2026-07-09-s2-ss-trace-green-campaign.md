...
- [ ] **Step 3: Implement.** Add `PRE_ROLL` as the intro's first phase with the Task-1 length; transition to DROP at the modeled tick. In `Sonic2SpecialStageManager.update()` wrap the `drawingIndex++` (`:1030`) and `trackAnimator.update()` (`:1043`) in `if (!intro.isPreRollActive())` (delete/replace the "Track animation always runs (even during intro)" comment — it is now wrong). Animator default speed factor 0; manager applies 12 at the modeled `:6640` tick via `setSpeedFactorForInit(12)`. The intro phase machine is already rewind-snapshotted (`:2149`) — extend the snapshotted enum mapping if it serializes ordinals/names.
...
- [ ] **Step 3: Implement.** `spawned` flag default false, set true for all players at the Task-1 spawn tick (manager-driven, keyed off the pre-roll phase end per the timeline — the ROM writes object ids at `:6628-6634`). Gate `updatePlayers()` participation and renderer submission on it. `toComparisonPlayerState(player)` returns null when `!player.isSpawned()`. Snapshot: add the boolean to the player section of `Sonic2SpecialStageSnapshot` capture/restore.
...
- [ ] **Step 2: Quantify the collapse.** From the new report: total errors/warnings, first-error frame+field, first divergence per field (the Task-14 python one-liner pattern lives in the campaign spec's Ground-truth section — reproduce it). Compare against the baseline (15,313 / 1,877 / first error f0). Record before/after in the frontier log with the two commit SHAs.
