...
- [ ] **Step 1:** Failing test — construct the manager RAW (`new Sonic3kSpecialStageManager()`, NO `initialize()` call: the offsets-unverified placeholder branch is DEAD CODE — `Sonic3kSpecialStageRomOffsets.areOffsetsVerified()` hardcodes true, so `initialize()` always takes the ROM path and needs `GameServices.rom()`), then seed state directly via the sub-object seams the way `TestSonic3kSpecialStageManagerSnapshot.java:70-73` does (`getPlayer().initialize(...)`, `getGrid().setCellByIndex(...)`). Call `captureComparisonState()`, assert every field equals its getter counterpart, and assert two consecutive calls with no update() in between are equal (pure-read proof).
...
- Test: extend `TestTraceSessionLauncherSsConfig` — call the now package-private `prepareSpecialStageConfiguration(meta)` directly: fresh_load=true metadata → S3K_SKIP_INTROS=false; fresh_load absent/false → untouched (this exercises the live meta-reading path, not just the explicit-boolean helper)
