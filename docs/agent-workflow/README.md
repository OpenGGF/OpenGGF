...
| `DezFinalRouteAuthorTool` | Observes the final DEZ encounter and authors controller-only input through hands, core and escape. Fixed declared solo Sonic boot: 200 rings, seven Super Emeralds. Writes input script/state CSV; no gameplay-state injection or parity claim. | `DezFinalRouteAuthorTool <ROM> <width> <output-prefix>` (same Java classpath as gameplay capture) |
