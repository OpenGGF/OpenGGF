...
### Pyramid presentation and entry-priority follow-up (2026-09-16)

User review explicitly authorized repeating existing pyramid tiles and stopping
widescreen scrolling before the foreground becomes a wall. Base is
`3998a70298fe5ed0e10a349bc53dec8f7b90bcbc`; development worktree is
...
- Baseline `TestSozPyramidWindow,TestSozScreenEvents`: 13 tests, no failures/skips.
- `python3 tools/testing/maven_queue.py -Dmse=off
...
- Final source strip: queued Maven with
  ...
  `-Dsoz.act1.victory.every=1` and the external capture directory:
  93 tests, no failures/skips.
- Provider/render consumers: queued Maven with
  `-Dtest=TestSonic3kZoneFeatureProvider,TestSonic3kActTransitionZoneFeatures,TestSozPostBossPlaneState,TestHczOverlayCommandPool,TestIncrementalBgTilemapWindow,TestLevelTilemapManagerRewindReset`
...
- `LUA_BIN=/usr/bin/lua5.4 python3 tools/testing/run_categories.py
  ...
  movement, victory, transition and rewind consumers. These are focused results,
