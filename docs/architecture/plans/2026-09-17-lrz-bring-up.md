...
## Handover, 2026-09-18 (twelfth)

**Head `213441fde`** plus this entry, branch `feature/ai-lrz-bring-up`, base develop `035e48a58`.
Tree clean; nothing pushed or merged.

**What landed, in order.**

1. `2e0c79bbf` -- `Obj_Toxomister`'s body is a `Touch_Enemy`-type badnik and can now be destroyed.
   This was the cold route's first divergence. **The route frontier moved 831 rows**, from native
   row 2323 to **row 3154**.
2. `1730f258a` -- the seamless `$900` -> `$901` act change, both halves.
3. `9b7716f4a` -- the independent review's blocker and four more findings applied; two rejected
   with the ROM.
4. `213441fde` -- the miniboss fight cycle measured, and what actually ends every attempt.

**The act change runs in production and is the round's main deliverable.** The whole chain from
`Obj_Results`' publication to `$901` is driven by `TestS3kLrzSeamlessActChangeHeadless`, which
asserts the act word, the stage the change parks on, the trigger-array clear, the player's landing
on `Camera_min_X_pos + $10`, the camera position, and -- the assertion the review's blocker needed
-- a carried object moving by exactly `-$2C00`. Removing `romWorldObjectOffsetRange` once
reproduces the blocker in that assertion alone.

**Read the two rejected findings before "fixing" them.** `loc_56CAA` allocates no
`Obj_TitleCard`, so `showInLevelTitleCard(false)` and an `IMMEDIATE` art admission are right and
`TITLE_OWNER` would never be consumed. And it clears `Dynamic_resize_routine`, so act 2's resize
owner installs its own camera bounds on the change frame -- which is why the bounds are not
asserted and the camera *position* is. Both are in the evidence log with their measurements.

**A cost worth knowing before another zone adopts this.** `ROM_WORLD_OFFSET_RANGE` throws for any
carried object that reports `render_flags` bit 2 without implementing `RomWorldPositionedObject`.
Three classes had to be given the contract before the change would run --
`Sonic3kPathSwapObjectInstance`, `SozSpriteMaskObjectInstance` (both shared) and
`LrzMinibossInstance`. The live set at the change is what has to be compliant, and it is only
known by running the change.

**What is owed, in the order the next round should take it.**

1. **Clips `31` and `32`, and `33` behind them.** Not blocked on the act change, which is proven
   in production. Blocked on surviving the fight: **a hit zeroes the ring count**, so `--rings`
   buys exactly one hit, and the first hit arrives at `v 524` -- during the hover, long before the
   first slam -- from the hands' shots. The cycle is measured (857 frames; windows at `v 936`,
   `1793`, `2650`; hittable for ~96 frames each; `f = v - 1`; input line `i` plays at `v = i + 2`).
...
5. **The route's new frontier**: engine frame 3155 = native row 3154. Native lands on a floor at
   `(4274,770)` (`air 0`, `rolling 0`, `y_vel 0`, `g_speed` taking `x_vel 89`); the engine falls
   through. Kill condition in the frontier log.

**Carried forward unchanged**: the `$1D`/`sub_42EC0` route exercise, cold-route rewind spots and a
route spot for `$99`, act 2 placements on an act 2 route, `loc_849D8`'s `Set_IndexedVelocity`
`d0` for a retired Fireworm segment, the direct-`$901` act 2 background art gap, slice 5's clip,
and `TestS3kSonicTailsLrzSegmentTraceReplay` (last measured at `51474c172`, frame 208
`tails_y_speed`, 6835 errors, **not re-run this round**).

**Media.** No new clip. The next clip number is still `31`. `inputs/lrz1-miniboss-fight-v8.txt`
added. The review report is `notes/lrz-seamless-act-change-review.md`.

**Verification at this head.** `TestLrz*`/`TestS3kLrz*`/`TestSoz*`/`TestS3kSoz*`/`TestS3kHpz*`/
`TestS3kDdz*`/`TestFbz*`/`TestS3kFbz*` plus the four mandatory S3K classes,
`TestEveryObjectRewindRoundTrip`, `TestRewindHarnessCoverageRatchet`, `SwScrlLrzTest` and the
path-swap/sprite-mask tests: **3719 tests, 0 failures, 8 skips**, all eight
`@EnabledIfSystemProperty` SOZ capture harnesses. `-Pguards test -B`: **669, 0 failures, 0 skips**,
run twice -- the first caught a raw `setDestroyed` over the object-lifecycle budget from the
toxomister commit, fixed by sharing the single destroy site rather than raising the budget.
Both new tests were broken on purpose once and failed on their own assertion. **This is focused
validation, not a suite pass**: no `run_categories.py` selection, and no trace fixtures beyond the
SOZ complete run the class pattern pulled in.
