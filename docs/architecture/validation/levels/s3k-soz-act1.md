...
| --- | --- | --- | --- | --- | --- |
| PARALLAX / HEAT SHIMMER | `SwScrlSoz`: ROM tables, seven fractional bands and independent FG/BG phases | Normal desert moving-camera capture; all-line boundary regression | Camera/frame reconstruction; existing production route restore/replay passes | 600 native rows / 134,400 scroll words match source-derived arithmetic; BG copies and art phase also agree | 320/528 engine scenes inspected; exact native pixel match and full-width breadth open |
| ANIMATED TILES / PALETTE | Corrected SOZ1 DMA/channel range `$330..$341`; new `AnPal_SOZ1` cycle via palette registry | All32 secondary-art phases and 49 palette passes checked against ROM | Six-pass timer/offset restore checked; quicksand/vine/mechanism world replay passes | Full six-tile secondary transfer; native phase/cadence corroboration | CPU-to-rendered output inspected; matched cadence/pixel sequence still open |
