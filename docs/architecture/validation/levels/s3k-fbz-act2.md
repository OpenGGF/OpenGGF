...
| Tails solo / Knuckles solo | `TestFbzNativeCharacterRoutes#everyNativeTeamCanEnterBothActsFromLevelSelectAtTheRomStart`; `everySoloNativeCharacterCanEnterTheTerminalBossGraphAlive` seeds authored starpost 6 then drives ordinary inputs | Full cold-start traversal, mandatory mechanics, boss defeat, capsule and outgoing transition absent; boss allocation alone does not prove defeat/completion |
...
not a broad-suite or pixel-parity pass. Tails/Knuckles-main complete traversals,
