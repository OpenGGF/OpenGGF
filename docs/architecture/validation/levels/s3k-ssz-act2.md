...
| CAMERA: act-2 controller `loc_59078` | `loc_59078` routines 0/4/8 | declared `$FF00` seed, no-emerald route | `TestS3kSszAct2FinaleHeadless#seededNegativeSignalRunsThePersistentCameraControllerIntoStageEight` | yes through routines 0/4 and the routine-8 wait: persistent load allocation, two gradual-swing zero crossings, special-V-int `$C`, accelerated oscillator, camera move to `$600`, low-byte trigger and stage-8 handoff | cold allocation; ending signal seeded because its owner is out of scope | controller swing/velocity plus runtime signal captured; restore and one-frame forward replay asserted | exact swing constants, two-zero gate, `$D0` alignment, `$600` target and routine progression matched | open | Add the shared-ending routine-8 rise beyond its `$18` gate when that campaign supplies the final owner |
| BG: parallax and column waves | `SSZ2_*DeformArray`, `word_58C80` (FG per-line HScroll), `loc_5904A` (20-column VScroll waves) | arithmetic at 320; wide render pending | `TestS3kSszScrollBands` | partial: cold `$1000` fan, fixed `$5E` background, BG deformation, `word_58C80` foreground bands and 20 VSRAM columns; ending-only routine `$18` mode pending | yes on cold load | runtime accumulator captured | cold constants, both deform arrays and scanline transitions source-matched | open | Add ending-only event mode and moving 320/wide inspection |
...
| BOSS: Mecha Sonic phase, forced run, Super phase, Master Emerald | `Obj_SSZEndBoss` act-2 init `($220,$4A0)`, `loc_7B8E6`..`loc_7BBE0`, `Obj_SSZ2_Boss` (36 routines), burst `loc_7C6F0`/`ChildObjDat_7D4A8`, missile pod `loc_7C78E`, laser `loc_7C744`/`loc_7C764`, emerald `loc_7C818` | 320 Knuckles | `TestS3kSszAct2FinaleHeadless`, `TestS3kSszMechaSpawnHeadless`, `TestSonic3kPlcArtRegistry` | yes through the accepted cold stop: act-2 init, fresh eight-hit Super phase, transformation/forced-run bridge, all 36 native dispatcher slots, eight-way harmful energy burst, mirrored ROM-animated missile pod, four-script attached/aimed laser sequence, ROM-art Master Emerald, arena/music handoff and save stop line | yes into distinct Super dispatcher | yes: bridge, burst children, missile pod, laser parent/state, emerald and graph fields captured; live graph capture/restore plus one-frame forward replay | route order, routine byte domain, waits, eight `word_7D172` velocity rows, pod offset/script, both laser scripts and dominant-axis `$400` aiming, emerald ROM addresses and the effects sheet's 26 owned mapping frames source-matched | open: ROM sheet now loads; moving inspection pending | Capture and inspect the effects in motion |
| PRESENT: palette rotation | `sub_7C678` installs `word_7D9EA`; `Run_PalRotationScript` | 320 Knuckles transformation | `TestS3kSszAct2FinaleHeadless#transformationRunsTheRomBackedPaletteRotationScript` | yes: transformation landing allocates a ROM-backed 16-colour script owner, including delay/repeat handling and palette ownership arbitration | yes through first-health defeat and transformation landing | script pointer/cursor/timer and parent identity captured; restore and one-frame forward replay asserted | pointer/header/destination/count and first ROM colour asserted | open | Add moving transformation-flash inspection |
| DEFEAT: save and stop line | `loc_7BCB0` (`Events_fg_4+1`, `object_control $83`, `SaveGame`), `loc_7BCFC` after `(2*60)-1` | 320 Knuckles | `TestS3kSszAct2FinaleHeadless#secondHealthBarLocksKnucklesAndPublishesTheEndingHandshake` | yes: second eight-hit terminal graph, `$BF`, `$1F` and 119-word waits, progression save request, player `$83` lock/frame clear, event handshake and accepted ending-object boundary | production route through first bar and Super dispatcher; terminal health reduction isolated from long harmful animation windows | boss phase, timers and player/event state captured | wait constants, control writes and stage-4 consumption source-matched | open | Add moving defeat/fade inspection; shared ending object remains out of scope |
...
| PRESENT: tile fill and ending island mask (seeded) | `SSZ2_ScreenEvent` stage 8 / `$C`, `loc_591D6` (`Map_KnuxEndingIslandMask`) | declared initial `Events_fg_4 = $FF00`; `loc_59078` supplies the later positive low-byte trigger | `TestS3kSszAct2FinaleHeadless` seeded controller and tile-fill cases | yes: the controller handoff plus sixteen solid-index-six tiles, eight-piece SAT mask, `$F..-1` delayed-row countdown and camera reset | **cold-blocked**: only the ending object supplies the initial trigger | controller, runtime countdown and dynamic mask captured | trigger chain, exact tile range/value, mapping geometry, row count and terminal camera writes source-matched | open | Add a seeded rendered inspection; cold owner remains the ending campaign |
