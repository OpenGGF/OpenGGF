...
| PRESENT: tile fill and ending island mask (seeded) | `SSZ2_ScreenEvent` stage 8 / `$C`, `loc_591D6` (`Map_KnuxEndingIslandMask`) | seeded positive stage-8 input `Events_fg_4 = $00FF` (the preceding camera controller consumes the plan's initial negative `$FF00` seed before supplying this low-byte trigger) | `TestS3kSszAct2FinaleHeadless#seededEndingTransitionFillsMaskTilesAndCompletesDelayedDraw` | yes: sixteen solid-index-six tiles, eight-piece SAT mask, `$F..-1` delayed-row countdown and camera reset | **cold-blocked**: only the ending object supplies the initial trigger | runtime countdown and dynamic mask captured | exact tile range/value, mapping geometry, row count and terminal camera writes source-matched | open | Connect and test the preceding seeded `loc_59078` routines, then add a rendered inspection |
