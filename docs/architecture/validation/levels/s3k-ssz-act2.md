...
| BOSS: Mecha Sonic phase, forced run, Super phase, Master Emerald | `Obj_SSZEndBoss` act-2 init `($220,$4A0)`, `loc_7B8E6`..`loc_7BBE0`, `Obj_SSZ2_Boss` (36 routines), `loc_7B996` (`mus_DDZ`) | 320 Knuckles | `TestS3kSszAct2FinaleHeadless`, `TestS3kSszMechaSpawnHeadless` | partial: act-2 init, fresh eight-hit Super phase, fade/spin/player-lock/arena-open/run/land/transform/forced-run bridge, all 36 native dispatcher slots, arena/music handoff and save stop line; projectile children and emerald child pending | yes into distinct Super dispatcher | bridge and graph fields captured; graph round trip pending | route order, routine byte domain and waits source-matched; attack child timing pending | open | Add the Super graph's laser/projectile children, Master Emerald and graph rewind spot |
