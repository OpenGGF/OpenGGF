...
| ROUTE (Sonic + Tails cold): `$B00` entry → results | Complete-run BK2 from movie frame 468982 (segment directory `ssz`, `zone_id 11`) | native 320 | `TestS3kDezColdRoutes` | partial ratchet: exact 1,741 frames; survives 5,352 frames, max x `$0FB5` | focused pass, 2026-09-19 | Native rides the `$4E` lift pad at `$0EB8,$0948` from about row 2,700; engine misses that ride and splits hard by row 2,856 |
