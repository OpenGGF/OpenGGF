...
| ROUTE (Sonic + Tails cold): `$B00` entry → results | Complete-run BK2 from movie frame 468982 (segment directory `ssz`, `zone_id 11`) | native 320 | `TestS3kDezColdRoutes` | partial ratchet: exact 3,624 frames; survives the 19,870-frame measurement cap, max x `$1587` | focused pass, 2026-09-19 | Prior drawn-head touch state aligns the Spikebonker hit, Chainspike probes from native `y_radius=0`, and the torpedo launcher uses its 8x16 render extent for the on-screen timer gate. The next player-position split is row 3,625: the engine gives Sonic one extra two-pixel conveyor carry on departure |
