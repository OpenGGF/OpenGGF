...
| ROUTE (Sonic + Tails cold): `$B00` entry → results | Complete-run BK2 from movie frame 468982 (segment directory `ssz`, `zone_id 11`) | native 320 | `TestS3kDezColdRoutes` | partial ratchet: exact 3,517 frames; survives 4,386 frames, max x `$1598` | focused pass, 2026-09-19 | The two-bit conveyor standing-mask fix closes Sonic joining Tails at row 3,434. The next player-position split is row 3,518: native Tails newly sets pad slot 16's second standing bit and reverses it; engine sidekick contact has already diverged, so the player misses that carry |
