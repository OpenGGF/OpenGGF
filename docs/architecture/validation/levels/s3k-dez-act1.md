...
Status: all 365 placements are concrete, the miniboss and seamless act change are implemented,
and the focused DEZ surface is green through `1f2b7e3d7c`. Cold route and visual/native
certification remain open; nothing below certifies the act.
...
| Claim | State |
| --- | --- |
| Implemented | Presentation, all 365 placed objects, reverse-gravity consumers, the eight-hit miniboss graph, results signal and synchronous `$B00` → `$B01` reload with the native coordinate rebase |
| Cold-reachable | The committed Sonic+Tails input route is exact for 1,741 frames and survives 5,352 frames to x `$0FB5`; it then dies after missing the native lift-pad ride around route frame 2,700. The complete route to miniboss/results remains open |
| Rewind-verified | Palette/event state and every new object/boss graph have focused round trips; the load boundary isolates history, but an empirical route rewind at the boundary remains open |
| Native behaviour matched | Not started; replay frontier measured at `035e48a58`, see the row below |
| Visually matched | Engine inspection only, at 320 and 800 px (`raw-01-presentation-320`, `raw-02-presentation-800`, clips `01a`-`01d`); no native pixel comparison yet. Slice 4: clip `040-chainspike-800` shows the `$A5` charge, stop and return at 800 px; the `$61` launch and panels are still unfilmable behind the corridor jam |
...
| PLACEMENT: 365 act 1 objects | [inventory](../../research/s3k-zones/dez-object-inventory.md) | — | `TestS3kDezPlacementCensus` | 365 concrete / 0 placeholder | pass, `949c7e2aba` | Route/visual breadth remains separate from registration completeness |
...
| GRAVITY: `$5F` turbine corridor, 1 act 1 placement (`$2480,$0840`) | `sub_4964A`, sonic3k.asm:95814-95952 | — | `TestS3kDezGravityRoomHeadless` | implemented (`S3kDezGravityRoomObjectInstance`) | pass 8/8 | Nine mechanisms, nine breaks; three of them were silent on the first round and the tests were reworked until each reddened |
| GRAVITY: `$61` gravity puzzle, 1 act 1 placement (`$2690,$0840`, inside the `$5F` corridor) | `Obj_DEZGravityPuzzle`, sonic3k.asm:96087-96245 | — | `TestS3kDezGravityPuzzleHeadless` | implemented (`S3kDezGravityPuzzleObjectInstance`) | pass 12/12 | Twelve mechanisms, fifteen deliberate breaks, every one red. The six panel bits live in `S3kDezZoneRuntimeState` because the ROM keeps them in `MHZ_pollen_counter`, a level RAM byte. Filmed standing on the solid top through a full bob (`036-gravity-puzzle-320`); the launch and the panel markers are **unreachable in play** because the `$5F` corridor jams at x ≈ `$2636` ([known bug](../../../status/s3k-known-bugs.md#death-egg-act-1s-turbine-corridor-jams-at-x--2636)) |
| GRAVITY ROOM: `$60` `Obj_DEZBumperWall`, 10 act 1 placements in three shapes | `Obj_DEZBumperWall`, sonic3k.asm:95958-96082, launch `sub_49848`/`loc_49850` :96045-96080 | 320 | `TestS3kDezBumperWallHeadless` | implemented (`S3kDezBumperWallObjectInstance`) | pass 4/4 | Four assertions, eleven deliberate breaks. Subtype 0 is the `$17`x`$20`/`$21` wall (records 311/312), a positive subtype a `$13`-wide post whose height is the subtype itself (305/306 and 329/330 at `$18`, 327/328 at `$38`), and a negative one the room's exit gate (331/332), which moves itself to `$7F00` the update all six `$61` panel bits are set. **It does not clear the corridor jam** — see the [known bug](../../../status/s3k-known-bugs.md#death-egg-act-1s-turbine-corridor-jams-at-x--2636). No clip |
...
| BOSS: `$A6` miniboss | `word_7DDA4` range Y `$18C`-`$38C` X `$3400`-`$3780`; arena `$28C,$28C,$3680,$36C0`; 8 hits | — | `TestS3kDezMinibossInstance` | implemented, persistent across the seamless reload | pass, `1f2b7e3d7c` | Cold combat/transport capture remains open |
| EXIT: results-driven seamless `$B00` → `$B01` | `DEZ1_BackgroundEvent` `loc_593A8`/`loc_593EC`; X `-$3600`, Y `+$400`, event routines 0 | 320 | `TestS3kDezScreenEvents` | implemented with synchronous reload and explicit handoff | pass, `1f2b7e3d7c` | Full native-timed results/miniboss transport route remains open |
| ROUTE (Sonic + Tails cold): `$B00` entry → results | Complete-run BK2 from movie frame 468982 (segment directory `ssz`, `zone_id 11`) | native 320 | `TestS3kDezColdRoutes` | partial ratchet: exact 4,372 frames; corrected path survives 15,191 frames and reaches x `$1657` | focused pass, 2026-09-19 | Live standing-bit ownership closes the slot-7 invisible-block / slot-16 conveyor handoff. Gravity tubes now relinquish their per-player ride state when `RideObject_SetRide` transfers the live latch, and rolling remounts apply `Player_TouchFloor`'s native centre-radius delta. The former 19,870-frame cap was a stale-tube branch; the corrected path's next position split is row 4,373: native `$0DCB,$070F`, engine `$0DCA,$070F` |
...
| REWIND: cycle counters, event routine words | Registry restore equals capture plus forward replay | 320 | `TestS3kDezPresentationRewind` (2 tests) | implemented | pass, `1f2b7e3d7c` | Empirical load-boundary route spot remains open |
