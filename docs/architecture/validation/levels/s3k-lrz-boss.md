...
| PRESENT: animated tiles and palette | `AnimateTiles_LRZ3` channel 0 only at tile `$170` (`loc_2833C` returns for `Current_zone $16`); `$1600` AniPLC entry is `AniPLC_NULL`; `AnPal_LRZ3` gate `Palette_cycle_counters+$00` in {0, `$80`, 1} | native | `TestS3kLrzPatternAnimation`, `TestS3kLrzPaletteCycling` | implemented, including rewind state for the flash gate | focused pass | Native pixels remain open |
| EVENT: Death Egg flash sequence | `Obj_LRZ3Autoscroll` `loc_78F82` and `loc_793E2..loc_794D8`: 60-frame lock, six-frame flash animation at delay 4, eight-step fade, palette gate `$80` -> 1, bridge at `($60,$4D0)`, then `$11F`/`$BF` ship and debris waits | native | `TestLrz3AutoscrollController` | route signals and bridge implemented | focused pass | Flash, ship and debris sprite composition remains visual work |
...
| BOSS: end boss and lava surface | `Obj_LRZEndBoss` `collision_property $E` (14 hits), `off_79812` six routines; `Obj_59FC4` `SolidObjectTopSloped2`, push `Events_bg+$14`; shared `HScroll_table+$110` table | native | `TestLrzEndBossInstance`, `TestS3kLrzScrollRegistrationHeadless` | exact arena gate, 14-hit six-routine jump loop and sloped surface implemented | focused pass, pending revision | Projectile/child sprite composition and native pixels remain visual verification |
| LOAD: defeat -> capsule -> `$1601` handoff | `loc_79998`/`loc_79A30`, `mus_LRZ2` fade, `$EC0` gradual, `StartNewLevel $2D` | native | `TestLrzEndBossInstance` (defeat state) | gradual `$EC0` release and `$1601` request implemented | focused pass, pending revision | Capsule interaction/presentation and strict route replay remain |
