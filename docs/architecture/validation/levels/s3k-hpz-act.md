...
| ROUTE (Sonic + Tails cold): `$1601` entry → lower teleporter → upper corridor → fight floor | Complete-run BK2 controller input from movie frame 441758 (row `$1E09`, first Level_frame_counter) | native 320, Sonic + Tails | `TestS3kHpzSonicTailsColdRoute#recordedInputsReachTheKnucklesFightThroughTheLowerTeleporter` | implemented to the fight floor | pass | First blocker: Knuckles fight (inputs then desynchronise without the fight); extend through the fight, collapse and `$A00` when the fight lane lands |
