...
| ROUTE (Sonic + Tails cold): `$1601` entry → level intro run-in → lower teleporter → upper corridor → first Knuckles hits | Complete-run BK2 controller input from movie frame 441758; native `probe-route`/`probe-fight` positions | native 320, Sonic + Tails | `TestS3kHpzColdRoutes#recordedInputsReachTheKnucklesFightThroughTheLowerTeleporter`, `#recordedInputsMatchNativeCheckpointsThroughTheFirstKnucklesHits` | implemented; native behaviour matched (camera and both players every frame 441761-445002) | pass | First difference Player_2 at 445002 (fight); extend through the fight, collapse and `$A00` after merging the fight lane. Camera X trails native for 2 load frames (history from the previous act) |
