...
Placement baseline (`TestS3kLrzPlacementCensus`): 455 placed objects, of which **84 still build a
...
two orbiting spike balls, `$2B` (12) and `$2C` (40), and 84 after its `$29` flame throwers (52));
281 live rings (282 records minus the
...
| OBJECT: traversal families `$25 $2B $2C $2D $32 $37` (134 placements) | `Obj_LRZOrbitingSpikeBall*` (done), `Obj_LRZSolidMovingPlatforms` (`off_258BC`, `byte_25826`), `Obj_LRZTurbineSprites`, `Obj_LRZSpikeBallLauncher`, `Obj_LRZChainedPlatforms` | native | — | `$2B`/`$2C` implemented; `$25 $2D $32 $37` not | open | Slice 7. `$2D` is read out: `byte_25826` has two entries ({`$20,$20,0`}, {`$20,$20,1`}) selected by `(subtype >> 2) & $1C`, `off_258BC` has nine movers -- an `rts`, two `Oscillating_table` offsets on x (`+$0A - $20`, `+$1E - $40`), the same two on y, and four `sub_25974` ramps (`$5F`/`$7F` limits, `$40` stepping by 4) -- and `status` bit 0 negates every one |
