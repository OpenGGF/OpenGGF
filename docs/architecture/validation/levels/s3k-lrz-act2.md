...
Placement baseline (`TestS3kLrzPlacementCensus`): 455 placed objects, with **zero remaining
`PlaceholderObjectInstance` placements** (281 at `035e48a58`, 277 after slice 1,
...
32 after its `$2D` solid moving platforms (52), then zero after the three remaining traversal
families); 281 live rings (282 records minus the
...
| OBJECT: traversal families `$25 $32 $37` (32 placements) | `Obj_LRZTurbineSprites` (18), `Obj_LRZSpikeBallLauncher` (9), `Obj_LRZChainedPlatforms` (3) | native | `TestLrzTurbineSpritesObject`, `TestLrzSpikeBallLauncher`, `TestLrzChainedPlatformObject` | implemented | focused pass | Route pixels and full breadth remain open |
...
| EXIT: boulder cutscene and `$1600` request | `Obj_LRZ2CutsceneKnuckles` `$AE` at `($38B0,$240)`, `CutsceneKnux_LRZ2`, `loc_63C14` (Y >= `$4C0`: `Act3_flag`, ring/timer/shield carry) | native, Sonic/Tails/Tails-alone | `TestS3kLrzPlacementCensus` (registration); generic rewind sweep | implemented | classification/rewind pass | Exact boulder art, mapping, terrain-bounce visuals and a directed production route remain |
| EXIT: Knuckles `Obj_StartNewLevel` `$B3` -> `$1601` | `Check_InMyRange word_86426`; word decode at `subtype` (SST `$2D` must be 0); `SaveGame` only for `Player_mode 3` | native, Knuckles | `TestS3kLrzStartNewLevelObject` | implemented | focused pass | Production Knuckles route and rewind spot remain |
