...
Status: all 494 placements, the seamless entry, bottom-up redraw and end-boss/final-arena
handoff are implemented and focused-green through `1f2b7e3d7c`. Route, visual and native timing
...
| Claim | State |
| --- | --- |
| Implemented | Presentation, reverse gravity and its placed writers/readers, all 494 placements, seamless-entry redraw, eight-hit end boss and `$1700` final-arena request |
| Cold-reachable | Direct `$B01` and the production `$B00` seamless reload are green. Seeded free play retains **1256 frames** of exact player/camera/ring parity; a complete input-driven route to the boss remains open |
| Rewind-verified | Event/redraw words, gravity writes and every new placed object/boss graph have focused round trips; an empirical route rewind through the act seam remains open |
...
| EVENT: `DEZ2_BackgroundEvent` bottom-up redraw | stages 0-1 `Draw_PlaneVertBottomUp` (`loc_59532`/`loc_59556`) | 320 | `TestS3kDezScreenEvents` | implemented with rewound `$0F` row counter | pass, `c74872a33b` | Native scanline/cadence capture remains open |
| PLACEMENT: 494 act 2 objects | [inventory](../../research/s3k-zones/dez-object-inventory.md) | — | `TestS3kDezPlacementCensus` | 494 concrete / 0 placeholder | pass, `949c7e2aba` | Route/visual breadth remains separate from registration completeness |
...
| BOSS: `$A7` end boss | `word_7F0BE` range, `word_7F0C6` arena `$218,$288,$3400,$34E0`, 8 hits, `sub_7F8A0` gravity | — | `TestS3kDezEndBossInstance` | implemented with `$1700` carry/request | focused pass, `949c7e2aba` | Cold combat and exact phase timing remain open |
| REWIND: event routine words | Registry restore equals capture plus forward replay | 320 | `TestS3kDezPresentationRewind` | implemented | pass, `1f2b7e3d7c` | Empirical seam/boss route spots remain open |
