...
| ROUTE: cold act 1 from the level start | Controller-driven from `($100,$20)`, no teleport; the `$05` push-break rock needs a spindash, the `$19`/`$04` door needs its `$1C` button, the `$08` spikes need a jump | native 320 | `~/Videos/OGGF/lrz-bring-up/inputs/lrz1-cold-route-v5.txt`, `lrz1-native-input-route.txt` | started | frontier **x 1909** hand-authored (v7); the native-input run matches native rows **0-856 exactly** at `398c34991` (was 0-636) and reaches x 4029 | Not a test yet; no route class. The first divergence is now native row 857, `player_y` 1228 against 1230 -- the exact `player_y_speed` negation near `($663,$4CC)` whose owner is not a `$17`-family placement and has not been identified. The frame-637 divergence is closed; see the [trace frontier log](../../../status/trace-frontier-log.md) |
