...
Placement baseline at `035e48a58` (`TestS3kLrzPlacementCensus`): 609 placed objects, of which
**239 still build a `PlaceholderObjectInstance`**; 331 live rings (332 records minus the leading
`(0,0)` sentinel). The baseline only ratchets down.
...
| PRESENT: parallax, bands and shake | `LRZ1_Deform`, `LRZ1_BGDeformArray` `$40,$20,$10x5,$100,$10x3,$20`, `LRZ1_BGDrawArray` `$B0,$100`, `ApplyDeformation` at `HScroll_table+$00C` | 320 + one wide | `SwScrlLrzTest`, `TestSonic3kScrollHandlerProviderActKeys` | slice 1 | open | — |
...
| EVENT: runtime state and rewind adapter | `Events_routine_bg/fg`, `Events_bg+$00..$14`, `LRZ_rocks_*` | native | `LrzZoneRuntimeStateTest` | slice 1 | open | — |
...
| OBJECT: lava blocks `$6E` (34 placements, 4 subtypes) | `Obj_InvisibleLavaBlock` -> `bset #4,shield_reaction` -> `Obj_InvisibleHurtBlockHorizontal`; `sub_1F58C` | native + fire shield | `TestSonic3kInvisibleHurtBlockHObjectInstance` | slice 1 | open | — |
...
Baseline media: `~/Videos/OGGF/lrz-bring-up/raw-00-lrz1-before/`.
