...
Placement baseline (`TestS3kLrzPlacementCensus`): 609 placed objects, of which **43 still build a
`PlaceholderObjectInstance`** after slice 3 and `$9A` (239 at `035e48a58`, 205 after slice 1,
...
| OBJECT: badniks `$99 $9A $9B` (74 placements) | `Obj_Fireworm`, `Obj_Iwamodoki`, `Obj_Toxomister`; `PLCKosM_LRZ` | native 320 | `TestIwamodokiBadnikInstance` | `$9A` implemented (32 act 1); `$99` and `$9B` open | partial pass, `72920cabc` | Clip `25`. `$9A` has no touch collision at all: it is a solid block with a fuse. `$99` and `$9B` remain slice 4; the `PLCKosM_LRZ` art rows already exist in `Sonic3kPlcArtRegistry` |
