...
| OBJECT: badniks `$99 $9A $9B` (74 placements) | `Obj_Fireworm`, `Obj_Iwamodoki`, `Obj_Toxomister`; `PLCKosM_LRZ` | native 320 | `TestIwamodokiBadnikInstance`, `TestToxomisterBadnikInstance` | `$9A` and `$9B` implemented (32 + 22 act 1); `$99` open | partial pass, `c45f35e74` | Clips `25` and `26`, plus `TestToxomisterBadnikInstance`. `$9A` has no touch collision at all: it is a solid block with a fuse. `$9B`'s mist is a player hook -- an eighth off the speed a frame and a ring a second, escaped by a spindash or six left/right reversals. `$99` remains slice 4; the `PLCKosM_LRZ` art rows already exist in `Sonic3kPlcArtRegistry`. Owed: a clip of the mist actually catching a player |
...
| ROUTE: cold act 1 from the level start | Controller-driven from `($100,$20)`, no teleport; the `$05` push-break rock needs a spindash, the `$19`/`$04` door needs its `$1C` button, the `$08` spikes need a jump | native 320 | `~/Videos/OGGF/lrz-bring-up/inputs/lrz1-cold-route-v5.txt`, `lrz1-native-input-route.txt` | started | frontier **x 1909** hand-authored (v7); the native-input run matches frames 0-636 exactly and reaches x 4029 before dying at frame 3649 | Not a test yet; no route class. The stall is phase, not content: no placed object lies in x 1830-2150 / y 1100-1500, and the vertical climb from x 1877 needs jumps that land where native lands. See the two divergences in the [trace frontier log](../../../status/trace-frontier-log.md) |
