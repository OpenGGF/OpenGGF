...
|---|---|---|---|---|
| 1 | **A runnable launcher/output policy** | Direct `run_bizhawk_lua.sh` outside the sandbox with an external `OGGF_OUT` works; `run_s1_audio_parity.sh` cannot (OR blocker 1). | Either `S1AudioParityTool.resolveSafeOutputRoot` accepts an external run root, or TraceChaser's `output_policy.py` admits a consumer `target/`; the wrapper must export `OGGF_INPUT_REPOSITORY_ROOT`/`OGGF_WORKDIR`. | Every probe for every game; the S1 regression baseline. Cheapest item. |
| 2 | **Per-game Lua driver-state probe (S2, S3K)** — Z80 RAM image at the `zVInt` boundary plus YM/PSG writes attributed to that invocation | S1 only (`s1_audio_driver_parity_probe.lua`, 68k RAM `$F000..`, `event.onmemorywrite` on the chip ports; OR §3). | S2: read `1B80-1FF3` + `12FE-1307` (S2 §1, §3.1) after each `zVInt`; S3K: read `1C00-1FA0` (S3K §1.3-1.4) after each `zVInt` (twice on PAL double-update frames — NTSC first). Same write-callback mechanism; the PC hook is the Z80 `0038h` entry rather than a 68k routine. No native core needed. | The whole S2/S3K RAM+write oracle. Highest value: those games have **no** oracle of any kind today (OR blocker 3, 6). |
| 3 | **Engine-side ROM-vocabulary state producer** at `SmpsDriverServiceObserver.onServiceEnd` | `SmpsDriverSnapshot` after every tick; `S1AudioFieldRegistry`; reference-side normalisers for all three games with `normalizeEngine` uncalled (EM §5.1-5.2). | Per-game `SmpsDriverSnapshot → NormalizedState` mapping in `tools/audio/completerun/<game>/` (guard boundary, EM §5.2 item 10), the field classification of §1.2 #23, and a `ServiceEvent` per ROM update including delay frames (#1 above, EM §5.2 item 4). | Any comparison at all for S2/S3K; a RAM comparison (not just chip writes) for S1. |
...
| 5 | **S3K and S2 reference BK2s** | None (OR §6). | Sound-test movies first (music only, mirrors S1 GHZ), then one gameplay segment per game exercising music + SFX + DAC + a pause + a 1-up (the CD §13 listening rows LC-S1-1..5, LC-S2-1..6, LC-S3K1..6 are the scene list). AIZ1 is the S3K release slice. | Items 2-4 have nothing to run without them. |
| 6 | **Gameplay request/admission timeline probe per game** | S1 GHZ1 only (`s1_ghz1_gameplay_audio_timeline_probe.lua`; last result `ADMISSION_EXTRA` at frame 958, OR matrix). | S2/S3K equivalents hooking `sndDriverInput`/`Play_SFX`/`Play_Music` and the driver's dispatch labels (S2 §4, §7.2; S3K §4.1-4.3). | Tests §1.2 #4 (queue) and #5 (priority) directly, which the RAM probe sees only as consequences. |
...
The order is deliberate: 1 → 2 → 3 → 4 gives S2 and S3K a music oracle with the same
shape S1 already has, using only Lua and Java. Nothing on that path needs the native
observer core, the C# host, or a TraceChaser code change beyond the launcher policy.
...
| B | S2 and S3K sound-test BK2s; per-game Lua driver-state probes (Z80 RAM at `zVInt` + write callbacks); per-game parity profiles in the Java comparator; engine-side `NormalizedState` producers at `onServiceEnd` with a per-update `ServiceEvent`. First S2/S3K music comparisons — **expected red** at the first delay frame (§1.2 #2); that red is the measurement that authorises the implementation work. | A; spec sections 1-3 per game (§2 items 2, 3, 5) |
...
| D (optional) | Cycle-stamped S1/S2 writes; PCM taps for DAC; complete-run producers. | observer core install (OR blocker 2) — not on this workflow's critical path |
