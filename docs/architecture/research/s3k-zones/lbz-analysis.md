...
| `Obj_LBZFinalBossKnux` / `Obj_LBZFinalBoss2` | The handoff is registered, but `LbzFinalBoss2Instance` is currently an inert, invisible persistent object. Big Arm phases, collision, art/PLC, defeat flow, and rewind are absent, so authentic Knuckles LBZ completion is blocked. | BLOCKER |
...
3. Implement Knuckles' Big Arm from `Obj_LBZFinalBoss2`; the current inert handoff is a concrete route blocker, not a verification-only item.
...
- Knuckles final boss / Big Arm is not implemented beyond an inert handoff; port and validate `Obj_LBZFinalBoss2` against the disassembly and a Knuckles LBZ trace.
