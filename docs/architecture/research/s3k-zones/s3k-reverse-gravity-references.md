...
| 34038 | `loc_18C20` | `Obj_DashDust` (`loc_18C20`): status Y-flip and `-4` Y offset | `SpindashDustController.draw`: gravity mirror and signed Tails centre adjustment | covered |
| 34113 | `loc_18D14` | `Obj_DashDust` (`loc_18D14`): negates the skid-dust Y offset | `SkidDustObjectInstance.create`: signed per-character foot offset | covered |
...
| 41999 | `loc_1E44C` | `sub_1E410` (`SolidObjectTop` landing): reverse variant `loc_1E4D6` | Real DEZ spring contacts cover flat snap asymmetry, standing/rolling radii and exact X/Y acceptance edges. Direct sloped helpers bypass this gravity test and retain upright arithmetic; a separate regression verifies that distinction | covered |
...
| H. Solid objects and platforms | 6 | 5 | 0 | 0 | 1 |
...
spindash draw mirroring/Tails offset and skid spawn contact-side placement.
