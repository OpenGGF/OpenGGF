# Merge resolution record: origin/develop -> next

Point-in-time record of the conflict-resolution policy applied when merging
`origin/develop` into `next`. Captured while the merge was live in a local
`next` worktree; retained as provenance for how the shared runtime, next-only
features, and generated artifacts were reconciled.
