...
The persistent Sonic 2 PLC/DPLC queue is resumed by VBlank service: six patterns in
...
| Sonic 2 PLC/DPLC queue | `NATIVE_SERVICE_QUEUE_PENDING_REVIEW`; it remains `NATIVE_SERVICE_QUEUE` in this game's classification. | Stable ROM submission identity, proof of timing variation beyond lag/phase, an unambiguous production readiness edge, and a recorder boundary that distinguishes the completing entry from later queued work. |
