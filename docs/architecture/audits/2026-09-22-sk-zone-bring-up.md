...
| LRZ | Ten native-width cold Act1 routes now reach9915 inputs through crusher descent, dash elevators, upper spike platforms and a fire-shield gap crossing, with88 full-registry replay spots. Door7 requires a westward button detour next. Piece-release fallthrough is corrected; the next raw native-row difference6234 is a lag boundary, not an attributed movement defect. Turbines, chained platforms, cutscenes, boss act, end boss and HPZ handoff implemented in this campaign. A 12820-frame fresh boss-act route reaches HPZ. Act 1 miniboss runtime art and lava-arrival priority are corrected; retain distinct cold-route, strict-parity and breadth gaps. |
