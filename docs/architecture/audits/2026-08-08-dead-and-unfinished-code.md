...
| P1 (resolved 2026-08-08) | `Sonic3kCoordFlagHandler.handleMetaCommand(...)` | `SND_CMD`, `MUS_PAUSE`, `COPY_MEM` had operand-only cases. | ROM-backed sequencing inventories every shipped S&K/S3 music and SFX table entry and reaches none of `FF 01/02/03`; the handler now states the operand-alignment-only disposition without pretending native dispatch, halt, or shared-memory copy is implemented. Native contracts and the test command are recorded in `docs/architecture/research/audio/2026-08-08-s3k-smps-meta-command-reachability.md`. |
