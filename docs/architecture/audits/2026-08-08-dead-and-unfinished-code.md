...
| P1 (resolved 2026-08-08) | `Sonic3kCoordFlagHandler.handleMetaCommand(...)` | `SND_CMD`, `MUS_PAUSE`, `COPY_MEM` had operand-only cases. | A fixed-point ROM-backed graph inventory closes every resolved track entry (51 S&K music, 50 S3 music, 169 named SFX) with no unexplored frontier and reaches none of `FF 01/02/03`. Native dispatch is enumerated through `DF` (`DC` is S&K CreditsK music; `DC-DF` are S3 SFX/aliases), while aliases are not additional named loader streams. `EB` conditional loop-exit is a separate implemented path. Native contracts and the test command are recorded in `docs/architecture/research/audio/2026-08-08-s3k-smps-meta-command-reachability.md`. |
