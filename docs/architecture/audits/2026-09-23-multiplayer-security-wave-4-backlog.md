...
| --- | --- | --- |
| Prove predeadline completion during finish grace | The host can keep an already-active attempt open for the advertised two-second finish transit window, but the current finish message has no trusted completion timestamp. Arrival time alone cannot distinguish predeadline completion from completion during grace. The client sends a finish regardless of its local phase estimate, so the grace also acts as up to two seconds of extra completion time for an attempt running at the deadline. | Trace normal client frame and finish ordering around the deadline. Evaluate a host-observed finished frame or other authenticated protocol evidence without trusting a client-supplied clock; assess compatibility and verified-room behavior before changing the wire format. |
| Correlate asynchronous master replies without sacrificing availability | FIFO tombstones prevent a timed-out request's late response from completing a later request. If no response to the earlier request arrives at all, the off-by-one persists: each later reply of that type is consumed by the previous request's tombstone, so every later request of that type times out until the master connection is replaced. Post-wave-3 review found no remaining broker path that omits a list, create, or join reply, and the inbound rate limiter closes rather than drops. | Exercise timeout A/no A reply/request B/B reply for each master request type. Evaluate explicit request IDs across client and broker, including old-client compatibility and bounded pending state. |
