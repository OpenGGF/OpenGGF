...
## 2026-06-05 — S1 complete-run mid-run bootstrap: ghz1/mz1 sliced from one continuous TAS BK2

- Branch `bugfix/ai-s1-midrun-bootstrap`, worktree `.worktrees/s1-bootstrap` (off develop 6abc93c54).
- Recorded ghz1+mz1 (and all other acts) with the committed multi-segment
  `tools/bizhawk/s1_complete_run_recorder.lua` (v3.2) from
  `docs/BizHawk-2.11-win-x64/Movies/s1-complete-run.bk2` (Raiscan TAS, REV01). mz1 lands at
  BK2 offset 20791 (deep mid-run, `vblank_counter=4FEB` at frame 0); ghz1 at offset 788.
  Wired `TestS1Mz1CompleteRunTraceReplay` (zone 1 act 0) and
  `TestS1Ghz1CompleteRunTraceReplay` (zone 0 act 0).
- Command (worktree, cmd mvn.cmd):
  `mvn.cmd "-Ds1.rom.path=C:\Users\farre\IdeaProjects\sonic-engine\s1.gen" "-Dtest=com.openggf.tests.trace.s1.TestS1Mz1CompleteRunTraceReplay" -Dmse=off test`
- **Bootstrap finding: NO frame-0 blocker on current develop.** Both complete-run traces
  replay natively past frame 0 with `(no bootstrap divergences)`:
  - **mz1 complete-run: first error frame 1260** (`rolling`/`air` mismatch — ROM lands on a
    Monitor (obj 0x26) `air 1->0, on_object 0->1`, engine misses the monitor solid-box landing).
    Frames 0–1259 are byte-exact (x/y/speeds/camera).
  - **ghz1 complete-run: first error frame 1390** (`x_speed`; engine took a Crabmeat hurt
    (`rtn=04`) the ROM did not). Frames 0–1389 byte-exact.
  - Both first errors are deep-run object-physics divergences (monitor landing / badnik hurt),
    NOT mid-run bootstrap. The recorder's frame-0 arming convention
    (`bk2_frame_offset = emu.framecount()`, "return without recording frame 0") already makes
    the complete-run frame-0 row identical to the dedicated-bk2 frame-0 row, so the existing
    native bootstrap (`recordingStartFrameForTraceReplay = bk2FrameOffset`, start-position +
    ground snap) reaches the same grounded frame-0 state from the large mid-run offset.
- **Existing green S1 traces — status:** `TestS1Ghz1TraceReplay` GREEN; all 8
  `TestS1Credits0*TraceReplay` GREEN. `TestS1Mz1TraceReplay` (mz1_fullrun) is **pre-existing
  flaky-RED on clean develop** (verified on detached develop c7b017642: first error ~frame 3286
  `y_speed`, a non-deterministic Batbrain (obj 0x55) hurt-contact encounter; my worktree run
