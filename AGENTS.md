...
- **Mod API rule:** Mod API 2.4 is the current recursive `@ModApi` compatibility surface; 1.1, 1.2, and 2.0 through 2.3 remain frozen historical baselines. Version 2.2 added playable-subclass rewind hooks, 2.3 added the host-adapted S3K custom-zone adapter and game-owned palette bridge, and 2.4 adds exclusive game-start selection plus destination-scoped launch-team, deterministic input-filter, and HUD-profile policies. Character contributions use owner-tagged `CharacterKey` identities and the immutable module registry; standalone games use `AbstractStandaloneGameModule`, a durable `GameDataSource`, game-agnostic baked levels, and namespaced audio/save identities. Never bypass `ModContext` transactions or creator fault boundaries, trust a creator-reported runtime owner, allocate numeric mod ids, or make standalone content a ROM fallback. The Phase 4 creator contract, deterministic TMX subset, scheduled/parked scope decisions, and GUI non-commitment live under `docs/modding/index.md`; read it and `docs/architecture/mod-api-compatibility.md` before changing creator APIs, formats, converters, or examples.
...
Creator content uses the versioned `@ModApi` 2.4.0 surface (2.0.0 was a deliberate breaking bump from 1.1.0; 2.1.0 added additive ROM-art intake for S2 patch mods; 2.2.0 added playable-subclass rewind capture hooks — `AbstractPlayableSprite.captureSubclassRewindState()`/`restoreSubclassRewindState(...)`; 2.3.0 added the host-adapted S3K custom-zone adapter and game-owned custom-zone palette bridge; and 2.4.0 adds exclusive game-start selection plus destination-scoped launch-team selection, deterministic input filters, and row-only HUD profiles; see `docs/architecture/mod-api-compatibility.md`) and the two-artifact `ggfmod` toolchain. Code-bearing mods must remain namespaced, injected-service-only, rewind-recreatable, transactionally registered, and owner-fault-bounded. Phase 2 complete new zones still target Sonic 2 and preserve tagged zone identities rather than runtime indices; Phase 3 adds owner-tagged playable characters plus no-ROM standalone modules over durable bounded assets, game-agnostic levels, dynamic master-title entries, and namespaced slot-1 saves/audio. Phase 4 adds the hardened deterministic TMX path, source-first handbook, maintained seven-project gallery, exhaustive scope decisions, and a CLI-first GUI non-commitment. See `docs/modding/index.md`, `docs/architecture/mod-api-compatibility.md`, and `docs/modding/content-mods.md` before changing creator APIs, formats, converters, examples, callback ownership, or persistence.
