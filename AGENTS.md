...
- **Mod API rule:** Mod API 1.2 is a closed recursive `@ModApi` compatibility surface. Character contributions use owner-tagged `CharacterKey` identities and the immutable module registry; standalone games use `AbstractStandaloneGameModule`, a durable `GameDataSource`, game-agnostic baked levels, and namespaced audio/save identities. Never bypass `ModContext` transactions or creator fault boundaries, trust a creator-reported runtime owner, allocate numeric mod ids, or make standalone content a ROM fallback. The Phase 4 creator contract, deterministic TMX subset, scheduled/parked scope decisions, and GUI non-commitment live under `docs/modding/index.md`; read it and `docs/architecture/mod-api-compatibility.md` before changing creator APIs, formats, converters, or examples.
