...
Creator content uses the versioned `@ModApi` 1.2.0 surface and the two-artifact `ggfmod` toolchain. Code-bearing mods must remain namespaced, injected-service-only, rewind-recreatable, transactionally registered, and owner-fault-bounded. Phase 2 complete new zones still target Sonic 2 and preserve tagged zone identities; Phase 3 adds owner-tagged playable characters plus no-ROM standalone modules over durable bounded assets, game-agnostic levels, dynamic master-title entries, and namespaced slot-1 saves/audio. Phase 4 adds the hardened deterministic TMX path, source-first handbook, maintained five-project gallery, exhaustive scope decisions, and a CLI-first GUI non-commitment. See `docs/modding/index.md` and `docs/architecture/mod-api-compatibility.md` before changing creator APIs, formats, converters, examples, callback ownership, or persistence.
