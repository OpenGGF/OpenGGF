...
-- plan.watch_codes: optional code pointers; each frame the first SST slot running one of them is
-- watched instead (for objects whose slot depends on load order). Extra columns record the
-- watched object's status byte and y_vel, and Player_1's status byte.
local watch=0xB000+(plan.watch_slot or 48)*0x4A
local watchCodes={}
for _,c in ipairs(plan.watch_codes or {}) do watchCodes[c]=true end
local function watchAddress()
  if not next(watchCodes) then return watch end
  for slot=0,129 do
    local a=0xB000+slot*0x4A
    if watchCodes[mainmemory.read_u32_be(a) & 0xFFFFFF] then return a end
  end
  return watch
end
...
log:write('movie_frame,label,zone_act,lfc,cam_x,cam_y,p1_x,p1_y,p2_x,p2_y,bg_routine,fg4,shake,cycle0,fab8,pal4_c1,pal4_c2,pal2_c0,chaos,super,emeralds,watch_code,watch_routine,watch_x,watch_y,watch_hp,watch_status,watch_yvel,p1_status,p1_anim,p1_mapping,p1_objctl,pal1_c7\n')
...
while true do
  ...
  if cw then
    ...
    log:write(string.format('%d,%s,%04X,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%04X,%04X,%04X,%d,%d,%s,%08X,%d,%d,%d,%d,%02X,%d,%02X,%d,%d,%02X,%04X\n',
      ...
      mainmemory.read_u8(0xB020),mainmemory.read_u8(0xB022),mainmemory.read_u8(0xB02E),mainmemory.read_u16_be(0xFC0E)))
