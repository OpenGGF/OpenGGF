...
The coordinator prints `OPENGGF_TEST_RUN_START` and
`OPENGGF_TEST_RUN_END` markers containing the manifest path. The terminal
manifest is authoritative for `surefire_reports`, `trace_reports`,
`diagnostics_root`, `artifact_root`, and `distribution_root`; never read a
fixed `target/` report directory to certify a run.
