...
namespace_leader() {
    ...
    local leader_pid=$BASHPID leader_start leader_namespace
    ...
    leader_start=$(process_start "$leader_pid")
    leader_namespace=$(mount_namespace_inode "$leader_pid")
    [[ -n "$leader_start" && -n "$leader_namespace" ]] || die "could not record namespace leader identity"
    printf 'leader_pid=%s\nleader_start=%s\nmount_namespace_inode=%s\n' \
        "$leader_pid" "$leader_start" "$leader_namespace" > "$leader_info"
    printf 'ready\n' > "$ready_fifo"
    local barrier_value=
    ...
    shopt -u nullglob
    {
        ...
        printf 'namespace_leader_pid=%s\nnamespace_leader_start=%s\nmount_namespace_inode=%s\n' \
            "$leader_pid" "$leader_start" "$leader_namespace"
...
leader_pid= leader_start= leader_namespace= target_created=0 finalized=0
...
cleanup_target() {
    local holders current_start
    [[ "$target_created" == 1 ]] || return 0
    if [[ -n "$leader_pid" && -n "$leader_start" && -n "$leader_namespace" ]]; then
        current_start=$(process_start "$leader_pid" || true)
        if [[ "$current_start" == "$leader_start" ]]; then
            kill -TERM "$leader_pid" 2>/dev/null || true
            wait "$leader_pid" 2>/dev/null || true
        fi
        wait_namespace_gone "$leader_pid" "$leader_start" "$leader_namespace" || return 76
        holders=$(namespace_holders "$leader_namespace")
        [[ -z "$holders" ]] || return 76
    fi
...
on_exit() {
    ...
    trap - EXIT INT TERM
    (( finalized != 0 )) || cleanup_target || cleanup_status=$?
...
unshare --user --map-root-user --mount "$adapter" --namespace-leader "$namespace_token" "$@" &
leader_pid=$!
...
recorded_leader_pid=$(sed -n 's/^leader_pid=//p' "$leader_info")
leader_start=$(sed -n 's/^leader_start=//p' "$leader_info")
leader_namespace=$(sed -n 's/^mount_namespace_inode=//p' "$leader_info")
[[ "$recorded_leader_pid" == "$leader_pid" && "$(process_start "$leader_pid")" == "$leader_start" \
    && "$(mount_namespace_inode "$leader_pid")" == "$leader_namespace" ]] \
    || die "namespace leader PID/start/namespace identity changed before release"
...
printf '%s\n' \
    ...
    "parent_expected_empty=true" "parent_expected_mount=false" "leader_pid=$leader_pid" \
    "leader_start=$leader_start" "mount_namespace_inode=$leader_namespace" "build_root=$build" \
...
leader_status=$?
set -e
wait_namespace_gone "$leader_pid" "$leader_start" "$leader_namespace" || die "namespace leader or mount-namespace holder survived teardown"
