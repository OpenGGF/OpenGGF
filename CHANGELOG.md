...
- **S1 SpikedPoleHelix derives rotation phase from trace-seeded gfc (GHZ):** the GHZ Spiked Pole Helix (Obj 17, `Sonic1SpikedPoleHelixObjectInstance`) now computes its animation counter (`v_ani0_frame`) from the trace-seeded level frame counter every update, instead of maintaining a per-object counter initialised to 0 at construction (object stream-in). ROM `Hel_RotateSpikes` (`docs/s1disasm/_incObj/17 GHZ Spiked Pole Helix.asm:95-105`) reads the GLOBAL `v_ani0_frame` (RAM 0xFEC1), which `SynchroAnimate` ticks every 12 `v_framecount` (gfc) frames starting at gfc=1 (initial `v_ani0_time=0` underflows on the first call; `docs/s1disasm/sonic.asm:3111-3119`). A spike is harmful only when its per-spike frame `(v_ani0_frame + hel_frame) & 7 == 0`. The engine's per-object counter started at 0 when the helix streamed in mid-level, diverging from the shared global counter; at GHZ3 trace f4650 (gfc=0x1220), this placed one spike at harmful frame 0 while ROM had it at harmless frame 5, causing a spurious HURT (engine x_speed=-0x200 left/airborne vs ROM x_speed=0x0194 right/grounded). Fix: each `update()` derives `animCounter = (-((gfc + 11) / 12)) & 7` where `gfc = levelManager.getFrameCounter() + 1`, the same trace-seeded counter the Electrocuter fix uses (`v_framecount = Level_frame_counter`). Advances the S1 GHZ3 complete-run frontier **f4650 -> f5043** (363 -> 6 errors); object-local to Obj 17 (GHZ-only). Regression-free: GHZ1/GHZ2/SYZ2 stay GREEN, all other S1 frontiers byte-identical.
