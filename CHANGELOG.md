...
- **feat(rewind): generic round-trip harness is the Phase 2 rewind correctness gate.** `RewindRoundTripHarness` drives the REAL `ObjectManager.rewindSnapshottable()` → `RewindRegistry.capture()` → `RewindRegistry.restore(CompositeSnapshot)` path (not the tautological `captureRewindState()` + fresh-construction + `restoreRewindState()` path used by `RewindRoundTripProbe`). `TestEveryObjectRewindRoundTrip` adds: (1) a keystone test validating the DEZ boss + children round-trip with exact count and exact non-init scalar state; (2) a parametrized sweep over every discovered spawnable class — objects without a dynamic rewind codec return `Unprobed` (760 today, bounded by `UNPROBED_ALLOWANCE=800`), objects with a codec are fully round-tripped (count + scalar diff). `ObjectClasspathScan` widened to public; `RewindRoundTripProbe` delegates construction to `RewindRoundTripHarness.constructHeadless()` — one shared construction-strategy implementation. Real findings (not hidden): 4 parent-dependent child classes (`CnzMinibossCoilInstance`, `CnzMinibossSparkInstance`, `CnzMinibossTopInstance`, `GumballMachineObjectInstance$ExitTriggerChild`) fail because their codec's `recreate()` returns null when no parent boss exists in the isolated harness; these are documented gaps for Phase 2 follow-up.
