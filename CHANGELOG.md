...
- **Widescreen title-screen and title-card polish.** The S3K title screen now edge-extends its sea/sky background (Plane B) to fill the viewport instead of pillarboxing — out-of-frame screen columns clamp to the edge nametable column so the horizon, water, and sky run edge to edge while the centred foreground and overlay sprites (and the Tails plane flying across the now full-width sea) stay put. Title-card elements (S1 zone/act/oval sprites and the S2 red swoosh / yellow bar) now fully clear the screen on exit at widescreen widths: `TitleCardElement.setEdgeMargin()` extends each off-screen entry/exit endpoint outward by `viewportWidth - 320` so the centred composition slides on and off the real viewport edges rather than parking back inside the side bands. At native 320 px all of this is byte-identical (edge margin 0, exactly 40 background columns).
