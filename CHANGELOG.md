...
## Unreleased
- **S2 MTZ Twin Stompers Obj64 constructor priming now matches the ROM init fall-through:** `MTZTwinStompersObjectInstance` now applies exactly one `Obj64_Main` movement tick during construction for moving subtypes, matching `Obj64_Init` falling through into `Obj64_Main` before the next object dispatch (`docs/s2disasm/s2.asm:52699-52805`). The previous two-tick priming put the stomper 8 px too far into its extension cycle before the first relevant contact window. `TestS2MtzLevelSelectTraceReplay` advances from frame 1267 to frame 1840, where it now fails on a separate `g_speed` / contact frontier; `TestSonic2ObjectBugFixes` and same-game S2 ARZ/EHZ1/MCZ/SCZ guard traces pass.
