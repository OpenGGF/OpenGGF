...
| `AUDIO_FM_CORE` | `audio.fmCore` | string | `"accurate"` | Which FM synthesis core renders music and SFX. `"accurate"` is the cycle-exact Nuked-OPN2 port and the audio parity oracle; `"fast"` is the register-level clean-room core (about 5× cheaper per frame, not bit-exact). Audio parity captures always use the accurate core. |
...
audio:
  ...
  fmCore: accurate   # FM core: accurate (cycle-exact Nuked-OPN2) or fast (register-level clean-room core)
