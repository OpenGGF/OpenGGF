...
- **S3K Death Egg:** the Death Egg acts now hold their background still instead of scrolling it
  ...
  lift-pad arms use the high byte of their fixed-point angle, conveyor pads preserve their
  one-pass start and per-player direction-change timing, and Spikebonker, Chainspike and torpedo
  launcher contact/probe/visibility timing follows the native object slots. Conveyor carry also
  follows live standing-bit ownership when an earlier solid seats the player, while gravity-tube
  transfers release their former owner and restore the native rolling centre, carrying the recorded route through the machinery
