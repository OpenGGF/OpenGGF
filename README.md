...
- **Mod support:** Phases 0–3 provide patch composition, editor/export foundations, restart-scoped discovery, WAV/Ogg music overrides, trusted objects and reskins, complete Sonic 2 zones, tagged saves, Mod API 1.2 characters, and no-ROM standalone games. Phase 4 adds deterministic hardened Tiled TMX import, a source-first [creator handbook](docs/modding/index.md), and a CI-built five-project sample gallery. Its exhaustive [backlog decision record](docs/modding/BACKLOG.md) schedules base-game streamed SFX overrides and separate Sonic 1/Sonic 3&K zone adapters; MP3, hot reload, and the other parked items remain evidence-gated. The [GUI evaluation](docs/modding/GUI_TOOLING_EVALUATION.md) keeps `ggfmod` authoritative and makes no GUI build commitment.
