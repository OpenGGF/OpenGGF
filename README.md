...
- Project skill-set audit and repair (`bugfix/ai-skill-audit-fixes`): a full review of the 24 agent skills against the current source found and fixed stale copy-paste templates that no longer compiled (s3k-palette-cycling's retired `GraphicsManager` tick signature -> the real `PaletteOwnershipRegistry`/`S3kPaletteWriteSupport` path; s3k-zone-events' removed `Camera` constructor; boss factory snippets teaching `getInstance()`), a fully duplicated `s1-implement-object/rom-pitfalls.md`, contradictory frozen trace baselines (now deferred to `docs/TRACE_FRONTIER_LOG.md`), fabricated Sonic 1 disassembly filenames/RAM labels, rotted zone-status tables that would have re-dispatched four already-implemented parallax handlers, and two orchestration skills written for the wrong agent harness (now Claude Code-native in `.claude`, Codex-flavored only in `.agents`, with real `*TraceReplay` class names in both). The `.claude` tree was renamed to spec-standard `SKILL.md` and the `.agents` mirror regenerated from it with zero content drift across all shared skills.
- The S2 trace campaign closeout (`bugfix/ai-s2-trace-next`) drove the remaining Sonic 2 level-select expected-red frontiers to green — OOZ2 (Obj3E capsule body lifetime), ARZ2 (a long Obj89 boss-arrow/pillar/Egg-Prison chain), CNZ2 (Obj51 boss ball/flee/boundary chain), and finally MTZ3 — so **the full S2 level-select trace suite now passes (48/48)**. The MTZ3 endgame replaced accumulated compensations with five disassembly-verified fixes: the Obj54 boss and Obj53 orb-0 init frames each consume their first dispatch (the boss had run one float frame ahead of ROM for the entire fight), the whole `Obj54_AnimateFace` hit reaction and `Obj54_Defeated` transition apply after the current routine's movement, the orb break path runs its same-pass `Obj53_OrbitBoss` tail with a faithful `Ani_obj53` AnimateSprite port gating the ground bounce on mapping frame `$B`, attacked orbs expose cleared collision flags immediately (matching `Touch_Enemy_Part2`'s RAM writes), and the boss is persistent through its off-screen camera-opening flee. The last four errors uncovered a previously unmodeled ROM mechanic — the S2 impatient-wait blink/get-up input gate (`Obj01_MdNormal_Checks`), which skips the whole grounded update for ~3 frames when input arrives during deep idle — now modeled data-driven for S2 Sonic on plain terrain, with a BizHawk RAM probe as ground truth.
- S2 Death Egg ending handoff fix (`bugfix/ai-s2-dez-ending-white-screen`): defeating the final Death Egg Robot now reveals the ending cutscene instead of leaving the screen held white, and the final fade freezes level simulation so Sonic cannot briefly trigger pit-death audio while the ending mode callback is pending.
- Level-trigger rewind-coverage fix (`bugfix/ai-level-trigger-rewind-coverage`): spindash-triggered MGZ columns didn't reverse on rewind because `Sonic3kLevelTriggerManager`'s `Level_trigger_array` was never registered with the rewind registry — a backward seek left it stuck triggered while the consuming object's own fields correctly rolled back, so a rewind-recreated column snapped straight back to its post-trigger position. Fixed with `Sonic3kLevelTriggerStaticAdapter`, plus the identical fix for the S2 analog `ButtonVineTriggerManager`. A new architectural guard (`StaticStateRewindCoverageAnalyzer` + `TestStaticStateRewindCoverageGuard`) source-scans for this exact bug shape — a global static manager with mutable state and `reset()` but no rewind coverage — and running it found (and this branch also fixed) the same gap in HCZ's water-skim, water-tunnel, breakable-bar, and water-rush palette-cycle managers.
- Rewind recreate-link tolerance sweep (`bugfix/ai-recreate-link-tolerance`) hardened every remaining `recreateForRewind` that crashed the restore when a linked parent/sibling was legitimately absent (destroyed by the player or swept from the captured set) — SYZ boss spike, HTZ boss flamethrower/lava ball, S2 cog/MCZ brick/swinging-platform children, Aquis wing, gumball machine children now drop the orphan instead of throwing, mirroring the ICZ ice-column crash fix. A new Optional-returning link API forces absence handling at the call site, and a build-time throw-ratchet guard (`TestRewindRecreateLinkToleranceGuard`) fails on any new throwing recreate beyond a justified baseline.
- Held-rewind audio no longer leaks across level boundaries (`bugfix/ai-rewind-audio-history-arming`): the raw PCM rewind-history ring is a fixed-duration buffer independent of the logical rewind-frame reset, so holding rewind past a level's frame-zero floor could keep draining backward into audio recorded before the level loaded. `LiveRewindManager` now clears the ring on every level-load boundary, and PCM recording itself is armed only while a rewind consumer (held-key live rewind or a Trace Test Mode session) can actually use it — `AbstractSmpsAudioBackend.fillBuffer()` previously wrote every audio buffer into the ring unconditionally, regardless of whether rewind was enabled or possible. Follow-up (`bugfix/ai-rewind-archunit-collaborator-fix`): a full-suite run caught the arming calls reaching `GameServices` directly from `com.openggf.game.rewind`, one of the ArchUnit-frozen runtime-owned framework packages — rerouted through `RewindController`'s own constructor-injected `AudioManager` collaborator instead, with no behavior change.
- ICZ ice-column rewind crash fix (`bugfix/ai-icz-segment-column-rewind-break`): rewinding after push-breaking a column segment crashed restore with "Missing restored ICZ segment-column previous segment" — the recreate link lookup now tolerates destroyed (cascade-flag-bearing) and swept previous segments, with a regression test reproducing the exact break-then-rewind crash.
- LBZ end-boss defeat-phase rewind completeness (`bugfix/ai-lbz-defeat-ephemeral-recreate`) made the defeat debris, smoke puffs, and gradual-max-X camera extender reconstruct across a mid-defeat rewind via parent-mediated `recreateForRewind` helpers; debris render cosmetics became captured scalars so pieces round-trip byte-faithfully, and the explosion controller remains deliberately deferred per the cross-boss pattern; a combined camera+object rewind test proves the extender's incremental boundary accumulator and the captured `Camera_max_X` stay coherent across a mid-defeat restore (no double-apply, no lost extension).
- Guard-baseline reconciliation (`bugfix/ai-mhz-guard-baseline-debt`) turned the default test suite fully green again after the MHZ trace-fleet wave: the Madmole side-drill's per-frame scratch player ref moved from a per-object `@RewindTransient` annotation to a central rewind-policy TRANSIENT entry, and the eight raw subpixel-preserving player position writes in the MHZ swing bars and Madmole carry migrated to the `NativePositionOps` seam — both proven behavior-neutral (byte-identical writes; guard A/B against clean develop).
- Donated data-select preview fix (`bugfix/ai-donated-preview-camera-resync`): S1/S2 save-select thumbnails donated into the S3K UI were showing only background art, with no foreground terrain or objects/badniks. The preview-capture code jumped the camera to the shot position and drew a single frame with no per-frame update tick in between, so the parallax foreground-Y cache and the object placement window — both only refreshed by that tick — stayed anchored to the load-time camera position. Fixed by resyncing both right after the camera move, reusing the existing rewind-restore camera-resync hooks.
- The rewind state-coverage wave (`bugfix/ai-mgz-top-rewind-grab-state` + four agent branches) fixed the user-reported MGZ spinning-top freeze — rewinding mid-ride recreated the platform with an empty grab map while the player stayed object-controlled with no owner — and then closed the whole bug family it exposed: per-player ride/carry state silently dropped from rewind keyframes when a class falls off the compact capture schema (or is skipped within it). Fixed with prove-first tests: MGZ top, MHZ swing bars, Pachinko magnet orb, CNZ barber pole/giant wheel/wire cage/vacuum tube, HCZ spinning column, S2 CPZ spin tube/speed launcher/flipper/springboard, S3K cork floor, AIZ collapsing bridges + transition floor + ride-vine chains, CPZ boss pipe segment links, and full LBZ end-boss child-graph reconstruction; non-final animation-state fields no longer knock ~20 classes onto the lossy generic path, and the player's MGZ carry-owner reference is relinked by a new post-restore pass. Two new architectural guards keep the class of bug extinct: a closed-world field-disposition guard (every object field must be captured by its class's actual capture path, explicitly transient/deferred, or structural-final — 96 pre-existing debt keys baselined, 27 closed by this wave) and a policy-reachability guard covering every CAPTURED field only the compact path can carry.
- Gamepad menu/pause completeness (`feature/ai-gamepad-nav-and-pause-fixes`) closed the gaps found after the initial gamepad bindings landed: every title/credits/level-select confirm gate across S1/S2/S3K now accepts a gamepad press (previously only the master title screen did, so a controller stalled right after game select), the three level-select managers and the launch-options panel now support gamepad D-pad navigation via a new `InputHandler.isDirectionHeld()` helper, and gamepad Start was rewired from the silent ROM in-game pause to the visible "PAUSED"-overlay pause that Enter already drives.
- Gamepad bindings (`feature/ai-gamepad-menu-bindings`) added four hardcoded (not remappable) bindings on the primary connected pad, each mirroring an existing keyboard action so every call site picks them up automatically: the north face button (Y/Triangle) toggles debug movement mode, the left bumper (LB/L1) holds live rewind, the right bumper (RB/R1) advances one frame while paused, and the Back/Select/View button opens/closes the main menu's per-game options panel.
- Rewind rendering performance (`bugfix/ai-rewind-fg-tilemap-rebuild`) eliminated the `render.fg` hotspot during held rewind: the rewind post-restore hook no longer force-dirties the foreground tilemap every backward step (which rebuilt and re-uploaded the full level-width tilemap each rewound frame), and the AIZ2 ship-loop FG ring's leading-edge fill was generalized to a bidirectional window reconcile so the ring self-heals under backward camera motion — rewinding through the ship loop now costs the same few column fills as forward play.
- ICZ1 rewind boundary hardening (`bugfix/ai-icz-rewind-sidecar-guard`) fixed the Ice Cap Act 1 load-time crash caused by compact zone-event capture trying to serialize the live snowboard intro object without a rewind identity table. The snowboard intro handle is now treated as transient structural state owned by ObjectManager rewind, and the zone-event rewind schema guard now rejects identity-table fields in scalar-only sidecars.
- The MHZ trace-fleet run (`bugfix/ai-trace-s3k-mhz`) drove the Mushroom Hill complete-run trace frontier from frame 1 to frame 2986 across nine disassembly-verified fixes: the `Get_LevelSizeStart` MHZ1 camera/min-X `$C0` load override, replay-bootstrap preservation of the stateful mushroom-cap animation counter, cutscene-button landing width/anchor and cutscene-door inclusive right edge plus trigger-frame slide, swing-bar subpixel-preserving player writes, curled-vine contoured landing detection, the S3K TailsCPU interact-word watchdog despawn (`sub_13EFC`, modeled as a typed `ObjectInteractionRules` flag), and the Madmole arc-throw grab landing on its exact ROM frame. Each fix passed an independent adversarial verify against `sonic3k.asm` (one round used a BizHawk mid-frame write-PC probe for ground truth), with S1/S2 trace greens and all six sibling S3K zone frontiers confirmed neutral throughout; the remaining frontier (Madmole arm rebound animation cadence and mid-carry floor hit) is recorded in [`docs/TRACE_FRONTIER_LOG.md`](docs/TRACE_FRONTIER_LOG.md).
...
- Sonic 2 trace-bank closeout advanced CPZ2 and OOZ2 while preserving all Sonic 1 trace replay tests green; the remaining S2 expected-red frontiers are itemized in the trace frontier log for continuation on the next development branch.
...
- Sonic 2 Casino Night Zone Act 2 boss rendering now preserves Obj51's ROM null mapping frame and uses the disassembly's frame IDs for the ship, generators, zap overlays, propeller, and fired electric ball, restoring the missing ship body and projectile art.
...
- Live rewind now presents an authentic VHS picture-search effect (`feature/ai-vhs-rewind-shader`): an engine-owned post-process pass renders scrolling tear bands, scanline jitter, chroma bleed, tape dropouts, a head-switching strip, and vertical wobble while the rewind key is held, composed before any user CRT display shader with a 4-frame-attack/10-frame-release envelope. Ctrl+rewind rewinds at half speed and Shift+rewind at double speed (a third tear band fades in toward double speed, and the band scroll phase is CPU-integrated so speed changes never teleport the bands), with `rewind.vhsEffect` and `rewind.vhsTearBands` config toggles and configurable modifier keys.
...
- Scaled-window title/game transitions now reshape the viewport from the actual GLFW framebuffer size after launch display dimensions are applied, keeping high-DPI/full-scaled windows from rendering gameplay into only part of the framebuffer.
- Sonic 1 horizontal camera scrolling now matches the shipped ROM's `FixBugs=0` behavior: the leftward camera move is uncapped while the rightward move keeps its per-frame cap (S2/S3K still cap both directions), gated by typed `CameraRules`.
...
- Sonic 1 Scrap Brain spin-conveyor platforms now clear their spawned-latch when they scroll out of view, matching the ROM's `SpinC_OutOfRange`, so the conveyor cluster re-spawns when the player returns to it (previously the despawned cluster never re-appeared and the player fell through where it should have been).
- Sonic 1 Scrap Brain Ball Hogs and their cannonballs now apply gravity with the ROM's one-frame delay (`ObjectFall` adds the old velocity to the position, then increments velocity for the next frame), so a thrown cannonball follows the correct fall-and-bounce arc instead of arriving early.
- Sonic 1 Scrap Brain spinning platforms now gate their spin cycle on the level frame counter rather than the VBlank clock, and drop a riding player the moment the platform stops being solid (matching the ROM's `Spin_Spinner` gate and `bclr #3,obStatus` detach), so the player rides and falls off them on the correct frames. On detach the platform releases its solid-object latch so the rider goes airborne on the next frame via his own ground check (as in the ROM), instead of staying pinned to the no-longer-solid platform.
- Sonic 1 Star Light Zone fans now evaluate their vertical wind range against the player's position captured at the start of the object pass, matching the ROM's `ExecuteObjects` slot order, so a fan no longer pushes one frame early when the player rides a staircase that re-seats their Y.
- Sonic 1 Star Light Zone seesaw spikeballs now spawn into a higher object slot than the seesaw (matching the ROM's `FindNextFreeObj`) and apply their launch fall on the launch frame, so a spikeball landing on one end of a seesaw launches a standing player upward on the correct frame.
- Sonic 1 Star Light Zone seesaws now reject an exact-touch (zero-distance) top landing, matching the ROM's `SlopeObject` unsigned penetration gate (`cmpi.w #-16,d0 / blo`), so a player falling onto a seesaw seats on the correct frame instead of one frame early.
- Sonic 1 Star Light Zone circling platforms now use their full collision width for top-landing detection and join the standard moving-platform landing-surface model (matching the ROM's `Circ_Platform` passing `obActWid` to `PlatformObject`), so a player jumping onto one lands on the correct frame.
- Sonic 1 badniks and Scrap Brain saws now check their off-screen unload after moving each frame, matching the ROM's `RememberState`-after-movement order, so off-screen objects unload on the correct frame and stop holding object slots the ROM has already freed.
- Sonic 1 Star Light Zone staircases now execute from their highest reserved child slot — after the zone's fans, matching the ROM object slot order — so a fan's wind push lands before the ridden staircase re-checks the player, and the staircase releases the rider on the correct frame.
- Sonic 1 Final Zone boss now advances the RNG seed every frame of its intro wait state, matching the ROM's per-frame `v_random` increment, so the boss's cylinder/plasma attack pattern is selected from the correct seed. It also advances the seed by 7 on the boss's cylinder-attack side contacts (when Sonic rolls into the boss body), matching the ROM's `loc_19F50` increment.
- Sonic 1 Final Zone boss now exits its intro wait against the previous frame's camera position (matching the ROM's `ExecuteObjects`-before-`DeformLayers` order) and defers each cylinder's first extension to the frame after it activates, so the cylinder/plasma attack cadence aligns with the original.
- A player edge-balancing on an object now suppresses the look-up / look-down camera pan, matching the ROM's `Sonic_Balance` → `Sonic_ResetScr`, evaluated against the current frame's balance state so the camera eases back to the default offset instead of panning when a balancing player holds up or down.
- A player who rolling-jumps off the top of a monitor is now released into the air instead of being re-grounded the same frame, matching the ROM's `SolidObject` upward-velocity gate (`tst.w y_vel / bmi`), scoped to genuine jump-offs via the jump flag so it does not affect normal monitor landings.
- Sonic 1 Green Hill collapsing ledges now clear the rider's on-object and pushing status bits when a fragment collapses out from under a standing player, matching the ROM's `Ledge_FragmentPiece` `bclr #3/#5,obStatus`, so the player is re-seated onto the underlying terrain that frame instead of staying pinned 2px low at the ledge's last slope position.
- Sonic 1 boss-hit rebounds now halve the bounced-back velocity, matching the ROM's `React_BossHit` (`neg` then `asr` on both axes); the engine previously only negated (the Sonic 2/3K behavior), so a rolling player bouncing off a Sonic 1 boss recoiled at double the original speed. Gated per-game so Sonic 2 and 3K boss bounces are unchanged.
- Sonic 1 Star Light Zone elevators now report their full platform width for edge-balance detection, so a player standing centered on a rising elevator is no longer mistaken for balancing on its edge — which had wrongly suppressed the look-up/look-down camera pan while riding.
- Sonic 1 Green Hill boss wrecking ball now starts its swing only once the boss ship reaches its combat-reverse state, matching the ROM's `GBall_Base` gate, so the damaging ball no longer swings a boss-state early and clips the player where the original would not.
- Sonic 1 Final Zone boss now treats an exact right-edge touch as a hit, matching the ROM's inclusive `SolidObject` edge test (`bhi`), so a player who rolls into the boss body at the precise edge bounces on the correct frame instead of one frame late — clearing the entire cylinder/plasma combat section that followed.
- Sonic 1 monitors now clear their collision type once broken, matching the ROM's `Mon_BreakOpen` (`obColType = col_none`), so an already-broken monitor no longer preempts the touch-response check of an adjacent intact monitor — a rolling player breaking two monitors in a row now opens the second on the correct frame (this brings Sonic 1 to parity with the Sonic 2 and 3&K monitors, which already do this).
- Sonic 1 swinging platforms and spiked-ball chains now allocate one object slot per chain link, matching the ROM's `Swing_Main` chain-build loop, so the engine's object-slot occupancy lines up with the original. Previously the whole chain occupied a single slot, which shifted every later object's slot down and (in Scrap Brain Act 2) mis-slotted the Walking Bombs.
- Sonic 1 Walking Bomb shrapnel now despawns by the ROM's on-screen render bound rather than a wider camera margin, matching `Bom_Shrapnel`, so spent shrapnel no longer linger off-screen holding object slots the original has already freed — correcting downstream object-slot occupancy in Scrap Brain.
- Sonic 1 Buzz Bomber missile now only self-cancels when its parent is destroyed during its flare phase (matching the ROM's `Msl_ChkCancel`, which the active falling-missile routine never calls), so a launched missile no longer vanishes the instant the Buzz Bomber is popped — keeping object-slot occupancy aligned with the original (it had been freeing its slot ~840 frames early, shifting later objects down a slot and, in Marble Zone Act 3, mis-positioning a Batbrain so a rolling player's bounce was missed).
- Sonic 1 collapsing floors now drop a standing player into the air the moment the ridden fragment's collapse timer expires (matching the ROM's `CFlo_FragmentPiece` `bclr #3,obStatus`), and the collapse countdown is deferred one frame to match the ROM's routine-advance timing — so the player falls on the correct frame instead of staying grounded on a floor that has already given way.
- Sonic 1 swinging platforms now use their full collision width for landing and seat a landing player from the platform's pre-move position (matching the ROM's `Swing_SetSolid`, which runs the solid/seat before the oscillation move) — so a player walking between two abutting swinging platforms no longer falls through the gap or gets seated a couple of pixels off on the landing frame.
- Sonic 1 swinging platforms now give a player one final surface Y carry on the frame he walks off onto an abutting platform, matching the ROM's `Swing_Action2` (where `MvSonicOnPtfm` re-seats Y unconditionally after `ExitPlatform`) — so the hand-off between two adjacent swinging platforms keeps the player correctly seated instead of dropping him for a frame.
- Sonic 1 sideways spikes now use their spawn-origin X for the off-screen unload check, matching the ROM's `out_of_range spikes_origX`, so a horizontal-moving spike no longer despawns itself early as it slides away from its origin — its solid underside stays loaded to block a jumping player's head where the original does.
- Sonic 1 smashable blocks now preserve the player's centre position when he curls into a ball on smashing through them, matching the ROM (which shrinks the player's height without moving his position) — so the rebound launches from the correct height instead of a few pixels too high.
- Sonic 1 moving blocks now use their full collision width for landing detection, matching the ROM's `MBlock_Platform` (which passes `obActWid` straight to `PlatformObject` with no narrowing) — so a player landing near the edge of a moving block is caught instead of falling through.
- Sonic 1 moving blocks now reject the exact-touch (zero-penetration) landing, matching the ROM's unsigned `PlatformObject` band (`cmpi #-16,d0 / blo`), so a player whose feet are flush with the surface lands on the ROM-correct frame rather than one frame early — bringing the moving block in line with the seven other platform-family objects that already do this.
