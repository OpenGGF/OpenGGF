...
- Sonic 1 Final Zone boss now advances the RNG seed every frame of its intro wait state, matching the ROM's per-frame `v_random` increment, so the boss's cylinder/plasma attack pattern is selected from the correct seed.
