...
- **Reserved load-time profile boundary (2026-08-09):** `FAST` and `REALISTIC` remain preserved unfinished aliases with unchanged `NONE`/`PROFILED` fallbacks, but their warnings and completion plan now name the optional S3K Kosinski admission domain. S1/S2 Nemesis PLC cadence is already ROM-derived and game-owned, while S2 DPLC is a separate lifecycle; neither is silently folded into the S3K profile. Finishing `FAST` needs an approved safety policy, and finishing `REALISTIC` needs exact top-level direct measurements plus a confidence/context rule.
