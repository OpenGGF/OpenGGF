    ...
    @Test void captureVictoryAndHandoff() throws Exception {
        ...
        try(var session=new GameplayCaptureSession(settings);var csv=Files.newBufferedWriter(output.resolve("state.csv"))) {
            ...
            for(int frame=0;frame<6500;frame++) {
                ...
                csv.write(session.stateLine(frame,input)+","+GameServices.level().getCurrentAct()+","+(boss==null?-1:boss.phase())+","+(boss==null?-1:boss.routine())+","+events.backgroundRoutine()+","+(input==null?0:input.p1InputMask())+","+(input==null?0:input.p2InputMask()));csv.newLine();
