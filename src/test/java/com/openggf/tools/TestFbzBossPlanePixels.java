...
class TestFbzBossPlanePixels {
    ...
    void reversedPlanesSampleTheirOwnWorldCoordinates(int width) throws Exception {
        ...
        try (var session = new GameplayCaptureSession(settings)) {
            ...
            for (int i = 0; i < 30; i++) session.step(null);
            ...
            PlaneSamples initial = assertTilePixels(session);
            ...
            session.step(null);
            PlaneSamples moving = assertTilePixels(session);
            ...
            session.step(null);
            assertTilePixels(session);
        }
    }

    private static PlaneSamples assertTilePixels(GameplayCaptureSession session) {
        var state = S3kRuntimeStates.currentFbz(GameServices.zoneRuntimeRegistry()).orElseThrow();
        ...
        int cameraY = state.bossForegroundVScroll();
        // FBZ2_CloudDeform and loc_530F0: logical BG on physical A,
        // logical FG on physical B, with independent VSRAM and HScroll.
        int movingX = cameraX - 0x2600 - state.bossBackgroundOffsetX();
        int movingY = GameServices.parallaxOrNull().getVscrollFactorBG();
        int compared = 0;
        int front = 0;
        int rear = 0;
        int mismatched = 0;
