...
public class Sonic1SceneryObjectInstance extends AbstractObjectInstance implements SpawnRewindRecreatable {
    ...
    // Scen_Main copies the Scen_Values priority byte (docs/s1disasm/_incObj/1C GHZ, SYZ Scenery.asm:28):
    // fireball launcher base rows use 2 (line 47), the bridge stump row uses 1 (line 53).
    private static final int PRIORITY_BUCKET_FIREBALL_BASE = RenderPriority.bucket(2);
    private static final int PRIORITY_BUCKET_BRIDGE_STUMP = RenderPriority.bucket(1);
    ...
    public Sonic1SceneryObjectInstance(ObjectSpawn spawn) {
        ...
        if (subtype == SUBTYPE_BRIDGE_STUMP) {
            ...
            frameIndex = 1;
            priorityBucket = PRIORITY_BUCKET_BRIDGE_STUMP;
        } else {
            ...
            frameIndex = 0;
            priorityBucket = PRIORITY_BUCKET_FIREBALL_BASE;
        }
    }
