...
/** Obj_DEZMiniboss's Load_PLC $76 and Queue_Kos_Module; no synthetic readiness delay. */
final class DezEndBossArtState implements RewindStateful<DezEndBossArtState.Value> {
    ...
    void submit(ObjectServices services) {
        try {
            ...
            if (services.currentLevel() instanceof Sonic3kLevel level) {
                var plc = Sonic3kPlcLoader.parsePlc(rom, 0x76);
            ...
            if (physical != null) {
                ...
                physical.enqueue(rom, Sonic3kConstants.ART_KOSM_DEZ_END_BOSS_ADDR,
                        Sonic3kConstants.ARTTILE_DEZ_END_BOSS * 32);
            }
            try {
                ordinal = S3kRuntimeArtCoordinator.from(services).moduleQueue().queue(rom,
                        Sonic3kConstants.ART_KOSM_DEZ_END_BOSS_ADDR,
                        Sonic3kConstants.ARTTILE_DEZ_END_BOSS).ordinal();
