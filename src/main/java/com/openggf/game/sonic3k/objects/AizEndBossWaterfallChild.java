 ...
 * pre-incremented animation cursor. Subtype 0 installs ROM's one-dispatch
 * {@code Go_Delete_Sprite} marker at the {@code $F4} callback. Subtype 2
    ...
    private void animate(int[] frames) {
        ...
        if (animationIndex >= frames.length) {
            ...
            if (routine == STATE_DROP || getSpawn().subtype() != SUBTYPE_DROP) {
                // Both callbacks enter Go_Delete_Sprite; its operation marker
                // survives until Delete_Current_Sprite runs next dispatch.
                routine = STATE_DELETE_PENDING;
            } else {
