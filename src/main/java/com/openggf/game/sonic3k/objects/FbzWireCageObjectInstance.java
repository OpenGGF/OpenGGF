...
public final class FbzWireCageObjectInstance extends AbstractObjectInstance implements SpawnRewindRecreatable {
    ...
    private void updateHorizontal(AbstractPlayableSprite p,int i){
        ...
        if(!participants.flag(i,2)){
            ...
            NativePositionOps.writeYPosPreserveSubpixel(p,p.getCentreY()+gap+3);p.setOnObject(true);p.setAir(false);
    ...
    private void transferStandingOwner(AbstractPlayableSprite player) {
        Object previous=player.getLatchedSolidObjectInstance();
