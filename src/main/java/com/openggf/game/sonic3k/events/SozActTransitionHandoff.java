...
record SozActTransitionHandoff(int playerDeltaX,int playerDeltaY) implements SeamlessTransitionResourceHandoff {
    ...
    @Override public void transferAfterTargetInit(){
        ...
        for(var follower:GameServices.sprites().getSidekicks()){
        ...
        var registry=GameServices.paletteOwnershipRegistryOrNull();var level=GameServices.level().getCurrentLevel();
        ...
        try{
            for(int line:new int[]{0,2,3}){
                ...
                if(line==2)System.arraycopy(GameServices.rom().getRom().readBytes(0x560EA,22),0,target,2,22);
                if(line==3)System.arraycopy(GameServices.rom().getRom().readBytes(0x56100,30),0,target,2,30);
                registry.applyTargetPatch(owner,line,0,target);
                S3kPaletteWriteSupport.applyLine(registry,level,GameServices.graphics(),owner,
