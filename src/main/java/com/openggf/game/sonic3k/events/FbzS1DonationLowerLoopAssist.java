...
/**
 * S1-donation-only traversal assist for FBZ2's lower loop.
 *
 * <p>The locked-on route stops at {@code $095A/$0A6B} and releases a
 * {@code -$0B00} spindash to clear the loop. Sonic 1 donation deliberately
 * removes spindash, so an ordinary left run through that same authored
 * approach receives the equivalent one-shot speed floor. Native S3K and S2
 * donation never enter this path.
 */
final class FbzS1DonationLowerLoopAssist {
    private static final int APPROACH_MIN_X = 0x0940;
    private static final int APPROACH_MAX_X = 0x0980;
    private static final int APPROACH_MIN_Y = 0x0A50;
    private static final int APPROACH_MAX_Y = 0x0A80;
    private static final int REARM_MARGIN = 0x20;
    private static final int NATIVE_SPINDASH_RELEASE_SPEED = -0x0B00;

    private FbzS1DonationLowerLoopAssist() {
    }
    ...
    static void applyToMainPlayer(AbstractPlayableSprite player, FbzZoneRuntimeState runtime) {
        ...
        boolean spindashEnabled = rules.playerCapability().spindashEnabled();
        int currentSpeed = player.getGSpeed();
        Decision decision = resolve(
                runtime.s1DonationLowerLoopAssistState(),
                CrossGameFeatureProvider.isActive(), spindashEnabled,
                player.getCentreX() & 0xFFFF, player.getCentreY() & 0xFFFF,
                player.getAir(), player.isLeftPressed(), currentSpeed);
        if (decision.nextState() != runtime.s1DonationLowerLoopAssistState()) {
            runtime.setS1DonationLowerLoopAssistState(decision.nextState());
        }
    ...
    static Decision resolve(S1DonationLowerLoopAssistState state,
                            boolean donationActive, boolean spindashEnabled,
                            int playerX, int playerY,
                            boolean airborne, boolean leftPressed, int groundSpeed) {
        Objects.requireNonNull(state, "state");
        if (state == S1DonationLowerLoopAssistState.CONSUMED) {
        ...
        if (!isEligibleApproach(donationActive, spindashEnabled,
            ...
            return new Decision(groundSpeed, S1DonationLowerLoopAssistState.ARMED);
        }
        int resolvedSpeed = groundSpeed <= NATIVE_SPINDASH_RELEASE_SPEED
                ? groundSpeed : NATIVE_SPINDASH_RELEASE_SPEED;
        return new Decision(resolvedSpeed, S1DonationLowerLoopAssistState.CONSUMED);
    }
    ...
    private static boolean isEligibleApproach(boolean donationActive, boolean spindashEnabled,
        ...
        return donationActive && !spindashEnabled
                ...
                // The exact authored envelope supplies spatial authority; no
                // donor name, route identity, or frame number participates.
                && groundSpeed < 0;
    }

    record Decision(int groundSpeed, S1DonationLowerLoopAssistState nextState) {
        Decision {
