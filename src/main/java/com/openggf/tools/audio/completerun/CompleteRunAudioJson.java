...
final class CompleteRunAudioJson {
    ...
    private static FrameNativeDiagnostics frameNativeDiagnostics(JsonParser p)throws IOException{if(p.currentToken()==JsonToken.VALUE_NULL)return null;startCurrent(p,"frame native diagnostics");field(p,"services");List<FrontierService> services=frontierServices(p);field(p,"rawChipInventory");List<FrontierOwnedChip> chips=frontierOwnedChips(p);field(p,"rawSnapshotInventory");List<FrontierOwnedSnapshot> snapshots=frontierOwnedSnapshots(p);field(p,"resets");List<NativeResetDiagnostic> resets=nativeResets(p);field(p,"managedCorrelations");List<NativeManagedCorrelation> correlations=nativeManagedCorrelations(p);field(p,"rawAncestryTransitionInventory");List<FrontierOwnedAncestryTransition> transitions=frontierOwnedAncestryTransitions(p);end(p,"frame native diagnostics");return new FrameNativeDiagnostics(services,chips,snapshots,resets,correlations,transitions);}
