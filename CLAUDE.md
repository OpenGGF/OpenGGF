...
`com.openggf.game.patch` adds owner-tagged additive `GamePatch` decorators. The engine-owned `ModuleResolutionService` resolves enabled owners and prerequisites at every gameplay launch choke point while `WorldSession` keeps both the root and resolved module, preventing repeated-resolution double wrapping. Metadata failures disable an owner plus dependents; arbitrary creator-apply failures abort launch. Creator assets must enter through bounded `ModAssetRoot` and source-typed `LoadOp` paths. The maintained creator and compatibility contracts live in `docs/modding/index.md` and `docs/architecture/mod-api-compatibility.md`; dated design specs are historical provenance only.
